...
NetworkError MockTlsChannel::Initialize(const TlsConfig& ConfigIn)
{
    ...
    std::cout << "  - 证书文件: "
              << (Config.CertificateFile.empty() ? "无" : Config.CertificateFile) << std::endl;
    std::cout << "  - 私钥文件: " << (Config.PrivateKeyFile.empty() ? "无" : Config.PrivateKeyFile)
              << std::endl;
    std::cout << "  - CA证书文件: "
              << (Config.CaCertificateFile.empty() ? "无" : Config.CaCertificateFile) << std::endl;
...
void MockTlsChannel::AsyncHandshake(TlsHandshakeHandler Handler)
{
    ...
    {
        if (Handler)
            Handler(NetworkError::NOT_INITIALIZED, TlsHandshakeState::FAILED);
        return;
    }
    ...
    {
        if (Handler)
            Handler(NetworkError::NONE, TlsHandshakeState::CONNECTED);
        return;
    }
...
void MockTlsChannel::AsyncRead(char* Buffer, size_t BufferSize, TlsReadHandler Handler)
{
    ...
    {
        if (Handler)
            Handler(NetworkError::NOT_INITIALIZED, 0);
        return;
    }
    ...
    {
        if (Handler)
            Handler(NetworkError::CONNECTION_FAILED, 0);
        return;
    }
    ...
    std::thread(
        [this, Buffer, BufferSize]()
        {
            ...
            if (Handler)
                Handler(NetworkError::NONE, DataSize);
        })
        .detach();
}
