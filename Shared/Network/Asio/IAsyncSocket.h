...
// 统一的异步操作回调类型
using AsyncReceiveHandler = std::function<void(
    Helianthus::Network::NetworkError, size_t, Helianthus::Network::NetworkAddress)>;
...
public:
    ...
    // 基础连接操作
    virtual Helianthus::Network::NetworkError
    Connect(const Helianthus::Network::NetworkAddress& Address) = 0;
    virtual Helianthus::Network::NetworkError
    Bind(const Helianthus::Network::NetworkAddress& Address) = 0;
    virtual void Close() = 0;

    // 统一的异步操作接口
    virtual void AsyncReceive(char* Buffer,
                              size_t BufferSize,
                              AsyncReceiveHandler Handler,
                              CancelToken Token = nullptr,
                              uint32_t TimeoutMs = 0) = 0;
    virtual void AsyncSend(const char* Data,
                           size_t Size,
                           const Helianthus::Network::NetworkAddress& Address,
                           AsyncSendHandler Handler,
                           CancelToken Token = nullptr,
                           uint32_t TimeoutMs = 0) = 0;
    virtual void AsyncConnect(const Helianthus::Network::NetworkAddress& Address,
