...
// 连接管理
void PerformanceMonitor::RegisterConnection(const std::string& ConnectionId,
                                                 ...
                                                 uint64_t BytesProcessed)
{
    ...
    if (It == ConnectionMetricsMap.end())
    {
        H_LOG(Perf,
              ...
              "UpdateConnectionMetrics unknown id={}",
              ConnectionId);
        return;
    }
                                           ...
                                           const std::string& Protocol)
{
    ...
    H_LOG(Perf,
          Helianthus::Common::LogVerbosity::Verbose,
          "RegisterOperation id={} type={} proto={}",
          OperationId,
          OperationType,
          Protocol);
}
                                                ...
                                                uint64_t BytesProcessed)
{
    ...
    if (It == OperationMetricsMap.end())
    {
        H_LOG(Perf,
              Helianthus::Common::LogVerbosity::Warning,
              "UpdateOperationMetrics unknown id={}",
              OperationId);
        return;
    }
...
// 错误分类更新
void PerformanceMonitor::UpdateErrorStats(const std::string& ConnectionId,
...
void PerformanceMonitor::UpdateOperationErrorStats(const std::string& OperationId,
                                                   const std::string& ErrorType)
{
    std::lock_guard<std::mutex> Lock(OperationMetricsMutex);
    auto It = OperationMetricsMap.find(OperationId);
...
// 连接池管理
void PerformanceMonitor::UpdateConnectionPoolStats(const std::string& ConnectionId,
                                                   const ConnectionPoolStats& Stats)
{
    std::lock_guard<std::mutex> Lock(ConnectionMetricsMutex);
    auto It = ConnectionMetricsMap.find(ConnectionId);
    if (It == ConnectionMetricsMap.end())
...
// 获取统计
const std::map<std::string, std::unique_ptr<ConnectionMetrics>>&
PerformanceMonitor::GetConnectionMetrics() const
{
    return ConnectionMetricsMap;
}

const std::map<std::string, std::unique_ptr<OperationMetrics>>&
PerformanceMonitor::GetOperationMetrics() const
{
...
// Prometheus 导出
std::string PrometheusExporter::FormatMetric(const std::string& Name,
                                             double Value,
...
std::string PrometheusExporter::ExportConnectionMetrics(
    const std::map<std::string, std::unique_ptr<ConnectionMetrics>>& Metrics)
{
    std::ostringstream Oss;
    ...
    for (const auto& Pair : Metrics)
    {
        ...
        std::map<std::string, std::string> Labels = {{"connection_id", ConnectionId},
                                                     {"remote_address", Metrics->RemoteAddress}};

        // 基础指标
        Oss << FormatMetric("connection_total_operations", Metrics->TotalOperations.load(), Labels);
        Oss << FormatMetric(
            "connection_successful_operations", Metrics->SuccessfulOperations.load(), Labels);
        Oss << FormatMetric(
            "connection_failed_operations", Metrics->FailedOperations.load(), Labels);
        ...
        Oss << FormatMetric(
            "connection_min_latency_ms", Metrics->MinLatencyNs.load() / 1'000'000.0, Labels);
        Oss << FormatMetric(
            "connection_max_latency_ms", Metrics->MaxLatencyNs.load() / 1'000'000.0, Labels);
        Oss << FormatMetric(
            "connection_p50_latency_ms", Metrics->GetLatencyPercentileMs(0.50), Labels);
        Oss << FormatMetric(
            "connection_p95_latency_ms", Metrics->GetLatencyPercentileMs(0.95), Labels);
        Oss << FormatMetric(
            "connection_p99_latency_ms", Metrics->GetLatencyPercentileMs(0.99), Labels);

        // 吞吐量指标
        Oss << FormatMetric(
            "connection_throughput_ops_per_sec", Metrics->GetThroughputOpsPerSec(), Labels);
        Oss << FormatMetric(
            "connection_total_bytes_processed", Metrics->TotalBytesProcessed.load(), Labels);
        Oss << FormatMetric(
            "connection_total_messages_processed", Metrics->TotalMessagesProcessed.load(), Labels);
        ...
        // 错误分类指标
        Oss << FormatMetric(
            "connection_network_errors", Metrics->ErrorStatistics.NetworkErrors.load(), Labels);
        Oss << FormatMetric(
            "connection_timeout_errors", Metrics->ErrorStatistics.TimeoutErrors.load(), Labels);
        Oss << FormatMetric(
            "connection_protocol_errors", Metrics->ErrorStatistics.ProtocolErrors.load(), Labels);
        Oss << FormatMetric("connection_authentication_errors",
                            Metrics->ErrorStatistics.AuthenticationErrors.load(),
                            Labels);
        Oss << FormatMetric("connection_authorization_errors",
                            Metrics->ErrorStatistics.AuthorizationErrors.load(),
                            Labels);
        Oss << FormatMetric(
            "connection_resource_errors", Metrics->ErrorStatistics.ResourceErrors.load(), Labels);
        Oss << FormatMetric(
            "connection_system_errors", Metrics->ErrorStatistics.SystemErrors.load(), Labels);
        Oss << FormatMetric(
            "connection_unknown_errors", Metrics->ErrorStatistics.UnknownErrors.load(), Labels);
        ...
        Oss << FormatMetric(
            "pool_active_connections", Metrics->PoolStats.ActiveConnections.load(), PoolLabels);
        Oss << FormatMetric(
            "pool_idle_connections", Metrics->PoolStats.IdleConnections.load(), PoolLabels);
        Oss << FormatMetric(
            "pool_max_connections", Metrics->PoolStats.MaxConnections.load(), PoolLabels);
        Oss << FormatMetric(
            "pool_utilization", Metrics->PoolStats.GetPoolUtilization(), PoolLabels);
        Oss << FormatMetric(
            "pool_avg_wait_time_ms", Metrics->PoolStats.GetAverageWaitTimeMs(), PoolLabels);
        Oss << FormatMetric(
            "pool_exhaustion_count", Metrics->PoolStats.PoolExhaustionCount.load(), PoolLabels);
    }

    return Oss.str();
}

std::string PrometheusExporter::ExportOperationMetrics(
    const std::map<std::string, std::unique_ptr<OperationMetrics>>& Metrics)
{
    std::ostringstream Oss;
    ...
    for (const auto& Pair : Metrics)
    {
        ...
        std::map<std::string, std::string> Labels = {{"operation_id", OperationId},
        ...
        Oss << FormatMetric(
            "operation_successful_operations", Metrics->SuccessfulOperations.load(), Labels);
        Oss << FormatMetric(
            "operation_failed_operations", Metrics->FailedOperations.load(), Labels);
        ...
        Oss << FormatMetric(
            "operation_min_latency_ms", Metrics->MinLatencyNs.load() / 1'000'000.0, Labels);
        Oss << FormatMetric(
            "operation_max_latency_ms", Metrics->MaxLatencyNs.load() / 1'000'000.0, Labels);
        Oss << FormatMetric(
            "operation_p50_latency_ms", Metrics->GetLatencyPercentileMs(0.50), Labels);
        Oss << FormatMetric(
            "operation_p95_latency_ms", Metrics->GetLatencyPercentileMs(0.95), Labels);
        Oss << FormatMetric(
            "operation_p99_latency_ms", Metrics->GetLatencyPercentileMs(0.99), Labels);

        // 吞吐量指标
        Oss << FormatMetric(
            "operation_throughput_ops_per_sec", Metrics->GetThroughputOpsPerSec(), Labels);
        Oss << FormatMetric(
            "operation_total_bytes_processed", Metrics->TotalBytesProcessed.load(), Labels);
        Oss << FormatMetric(
            "operation_total_messages_processed", Metrics->TotalMessagesProcessed.load(), Labels);

        // 操作特定指标
        Oss << FormatMetric(
            "operation_partial_operations", Metrics->PartialOperations.load(), Labels);
        ...
        // 错误分类指标
        Oss << FormatMetric(
            "operation_network_errors", Metrics->ErrorStatistics.NetworkErrors.load(), Labels);
        Oss << FormatMetric(
            "operation_timeout_errors", Metrics->ErrorStatistics.TimeoutErrors.load(), Labels);
        Oss << FormatMetric(
            "operation_protocol_errors", Metrics->ErrorStatistics.ProtocolErrors.load(), Labels);
        Oss << FormatMetric("operation_authentication_errors",
                            Metrics->ErrorStatistics.AuthenticationErrors.load(),
                            Labels);
        Oss << FormatMetric("operation_authorization_errors",
                            Metrics->ErrorStatistics.AuthorizationErrors.load(),
                            Labels);
        Oss << FormatMetric(
            "operation_resource_errors", Metrics->ErrorStatistics.ResourceErrors.load(), Labels);
        Oss << FormatMetric(
            "operation_system_errors", Metrics->ErrorStatistics.SystemErrors.load(), Labels);
        Oss << FormatMetric(
            "operation_unknown_errors", Metrics->ErrorStatistics.UnknownErrors.load(), Labels);
    }
...
std::string PrometheusExporter::ExportSystemMetrics(const SystemMetrics& Metrics)
{
    ...
    Oss << FormatMetric("system_peak_memory_usage_bytes",
                        Metrics.ResourceStats.PeakMemoryUsageBytes.load());
    ...
    Oss << FormatMetric("system_file_descriptor_count",
                        Metrics.ResourceStats.FileDescriptorCount.load());
    Oss << FormatMetric("system_buffer_pool_usage", Metrics.ResourceStats.BufferPoolUsage.load());
    Oss << FormatMetric("system_buffer_pool_capacity",
                        Metrics.ResourceStats.BufferPoolCapacity.load());
    Oss << FormatMetric("system_buffer_pool_utilization",
                        Metrics.ResourceStats.GetBufferPoolUtilization());
