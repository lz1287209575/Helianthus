...
PooledBuffer::PooledBuffer(char* Data, size_t Size, bool IsPooled)
    : DataPtr(Data), BufferSize(Size), PooledFlag(IsPooled)
{
}
...
PooledBuffer::PooledBuffer(PooledBuffer&& Other) noexcept
    : DataPtr(Other.DataPtr), BufferSize(Other.BufferSize), PooledFlag(Other.PooledFlag)
{
...
// BufferPool 实现
BufferPool::BufferPool(const BufferPoolConfig& ConfigIn) : Config(ConfigIn), InUseCount(0)
{
    // 预分配初始缓冲区
