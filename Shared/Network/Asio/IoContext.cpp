...
void IoContext::Post(std::function<void()> Task)
{
    ...
    // bool IsCrossThread = (PostingThreadId != RunningThreadId) || (RunningThreadId ==
    // std::thread::id{}); UpdateWakeupStats(0.0, !IsCrossThread); // 延迟将在任务执行时计算
...
IoContext::TaskId IoContext::PostWithCancel(std::function<void()> Task, CancelToken Token)
{
    ...
    // 更新唤醒统计
    bool IsCrossThread =
        (PostingThreadId != RunningThreadId) || (RunningThreadId == std::thread::id{});
...
IoContext::TaskId
IoContext::PostDelayedWithCancel(std::function<void()> Task, int DelayMs, CancelToken Token)
{
    TaskId Id = NextTaskId.fetch_add(1);
    auto ExecuteTime = std::chrono::duration_cast<std::chrono::milliseconds>(
                           std::chrono::steady_clock::now().time_since_epoch())
...
bool IoContext::CancelTask(TaskId TaskId)
{
    ...
    {
        std::lock_guard<std::mutex> Lock(DelayedTaskQueueMutex);
        auto It = std::find_if(DelayedTaskQueue.begin(),
                               DelayedTaskQueue.end(),
...
WakeupStats IoContext::GetWakeupStats() const
{
    ...
    Out.AverageWakeupLatencyMs =
        (TotalWakeupsInternal > 0) ? (SumWakeupLatencyMsInternal / TotalWakeupsInternal) : 0.0;
...
void IoContext::RunBatch()
{
    ...
    while (Running || HasPending())
    {
        ...
        // 使用批处理轮询
        int Timeout = (NextDelay >= 0) ? static_cast<int>(std::min<int64_t>(NextDelay, 1))
                                       : 1;  // 降低等待以尽快处理取消
...
void IoContext::UpdateTaskBatchStats(size_t BatchSize, double ProcessingTimeMs)
{
    ...
    {
        double TotalTime =
            TaskBatchStatsData.AverageProcessingTimeMs * (TaskBatchStatsData.TotalBatches - 1);
...
size_t IoContext::CalculateTaskBatchSize() const
{
    ...
    {
        return std::min(TaskBatchConfigData.MaxTaskBatchSize * 2, static_cast<size_t>(128));
    }
