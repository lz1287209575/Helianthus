    ...
    void MessageProtocol::ProcessBuffer()
    {
        while (true)
        {
            ...
            else if (CurrentState == State::WaitingForMessage)
            {
                ...
                // 提取消息
                std::string Message(ReceiveBuffer.begin(), ReceiveBuffer.begin() + ExpectedMessageLength);
                
                // 移除已读取的消息内容
                ReceiveBuffer.erase(ReceiveBuffer.begin(), ReceiveBuffer.begin() + ExpectedMessageLength);
