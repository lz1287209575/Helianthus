...
public:
    // 导出连接指标
    static std::string ExportConnectionMetrics(
        const std::map<std::string, std::unique_ptr<ConnectionMetrics>>& Metrics);

    // 导出操作指标
    static std::string
    ExportOperationMetrics(const std::map<std::string, std::unique_ptr<OperationMetrics>>& Metrics);
...
private:
    static std::string FormatMetric(const std::string& Name,
                                    double Value,
...
public:
    ...
    // 连接池管理
    void UpdateConnectionPoolStats(const std::string& ConnectionId,
...
{
public:
    ScopedTimer(const std::string& OperationId,
                const std::string& OperationType,
                const std::string& Protocol = "")
        : OperationId_(OperationId), StartTime_(std::chrono::steady_clock::now())
    {
    ...
    void SetBytesProcessed(uint64_t Bytes)
    {
