...
private:
    ...
    struct AcceptExManager
    {
        ...
        AcceptExManager(SOCKET Socket,
                        AcceptResultHandler Cb,
                        size_t MaxConcurrent = 8,
                        size_t MinConcurrent = 2)
            : ListenSocket(Socket),
              Handler(std::move(Cb)),
              MaxConcurrentAccepts(MaxConcurrent),
              MinConcurrentAccepts(MinConcurrent),
              TargetConcurrentAccepts(MaxConcurrent),
              IsActive(true),
              LastAcceptTime(std::chrono::steady_clock::now()),
              AcceptCount(0),
              ErrorCount(0)
        {
        }
        ...
        void AdjustConcurrency()
        {
            ...
            auto TimeSinceLastAccept =
                std::chrono::duration_cast<std::chrono::milliseconds>(Now - LastAcceptTime).count();

            // 如果最近有连接被接受，增加并发数
            if (TimeSinceLastAccept < 100 && AcceptCount > 0)  // 100ms内有连接
            {
                TargetConcurrentAccepts =
                    std::min(TargetConcurrentAccepts + 1, MaxConcurrentAccepts);
            }
            // 如果长时间没有连接，减少并发数
            else if (TimeSinceLastAccept > 1000)  // 1秒内没有连接
            {
                TargetConcurrentAccepts =
                    std::max(TargetConcurrentAccepts - 1, MinConcurrentAccepts);
            }
