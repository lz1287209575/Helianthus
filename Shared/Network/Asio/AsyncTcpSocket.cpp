...
namespace Helianthus::Network::Asio
{
    ...
    void AsyncTcpSocket::AsyncReceive(char* Buffer, size_t BufferSize, ReceiveHandler Handler)
    {
        ...
        if (ProactorPtr)
        {
            ProactorPtr->AsyncRead(FdValue, Buffer, BufferSize, [this](Network::NetworkError Err, size_t Bytes){
        ...
        ReactorPtr->Add(FdValue, EventMask::Read, [this, Buffer, BufferSize](EventMask Event){
    ...
    void AsyncTcpSocket::AsyncSend(const char* Data, size_t Size, SendHandler Handler)
    {
        ...
        if (ProactorPtr)
        {
            ProactorPtr->AsyncWrite(FdValue, Data, Size, [Handler](Network::NetworkError Err, size_t Bytes){ 
                if (Handler) 
                {
                    Handler(Err, Bytes);
                }
