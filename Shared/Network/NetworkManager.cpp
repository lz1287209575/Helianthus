...
namespace Helianthus::Network
{
    ...
    NetworkError NetworkManager::BroadcastMessageToGroup(const std::string& GroupName, const Message::Message& Msg)
    {
        std::lock_guard<std::mutex> GroupsLock(GroupsMutex);
    ...
    NetworkError NetworkManager::RemoveConnectionFromGroup(ConnectionId Id, const std::string& GroupName)
    {
        std::lock_guard<std::mutex> GroupsLock(GroupsMutex);
        auto It = ConnectionGroups.find(GroupName);
        if (It != ConnectionGroups.end())
    ...
    NetworkError NetworkManager::ValidateAddress(const NetworkAddress& Address) const { return NetworkError::SUCCESS; }
