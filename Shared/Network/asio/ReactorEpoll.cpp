...
ReactorEpoll::ReactorEpoll()
    : EpollFd(epoll_create1(EPOLL_CLOEXEC)), Callbacks(), EdgeTriggered(false), MaxEvents(64)
{
    if (EpollFd < 0)
    {
        H_LOG(
            Net, LogVerbosity::Error, "epoll_create1 failed: {}({})", std::strerror(errno), errno);
    }
}
    ...
    if (epoll_ctl(EpollFd, EPOLL_CTL_ADD, static_cast<int>(Handle), &Ev) != 0)
    {
        ...
        if (AddErrno == EEXIST)
        {
            ...
            uint32_t ExistingMaskBits = 0;
            {
                ...
                if (It != RegisteredMasks.end())
                    ExistingMaskBits = It->second;
            }
            ...
            if (epoll_ctl(EpollFd, EPOLL_CTL_MOD, static_cast<int>(Handle), &Ev2) != 0)
            {
                H_LOG(Net,
                      ...
                      "epoll_ctl MOD failed fd={} mask={} err={}({})",
                      Handle,
                      static_cast<int>(Ev.events),
                      std::strerror(errno),
                      errno);
                return false;
            }
        ...
        else
        {
            H_LOG(Net,
                  LogVerbosity::Warning,
                  "epoll_ctl ADD retry failed fd={} err={}({})",
                  Handle,
                  std::strerror(AddErrno),
                  AddErrno);
            return false;
        }
    }
...
int ReactorEpoll::ProcessBatchEventsOptimized(const std::vector<epoll_event>& Events, size_t Count)
{
    ...
    for (size_t I = 0; I < Count; ++I)
    {
        ...
        if (Mask != EventMask::None)
        {
            ...
            {
                It->second =
                    static_cast<EventMask>(static_cast<int>(It->second) | static_cast<int>(Mask));
            }
            else
            {
...
void ReactorEpoll::UpdatePerformanceStats(size_t BatchSize, double ProcessingTimeMs)
{
    ...
    {
        double TotalTime =
            PerformanceStatsData.AverageProcessingTimeMs * (PerformanceStatsData.TotalBatches - 1);
...
size_t ReactorEpoll::CalculateAdaptiveBatchSize() const
{
    ...
    {
        return std::min(BatchConfigData.MaxBatchSize * 2, static_cast<size_t>(256));
    }

    // 如果处理时间较高，减少批处理大小
    if (PerformanceStatsData.AverageProcessingTimeMs > 1.0)
    {
        return std::max(BatchConfigData.MinBatchSize, BatchConfigData.MaxBatchSize / 2);
    }
