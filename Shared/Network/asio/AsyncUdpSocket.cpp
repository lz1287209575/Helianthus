...
namespace Helianthus::Network::Asio
{
    ...
    // 新的 UDP 特定异步操作方法
    void Helianthus::Network::Asio::AsyncUdpSocket::AsyncReceiveFrom(char* Buffer, size_t BufferSize, UdpReceiveHandler Handler)
    {
        ...
        if (!ReactorPtr)
        {
            if (Handler) Handler(Network::NetworkError::NOT_INITIALIZED, 0, Network::NetworkAddress{});
            return;
        }
        ...
        ReactorPtr->Add(SocketFd, EventMask::Read, [this, Buffer, BufferSize](EventMask Ev)
        {
            ...
            Network::NetworkError Err = Socket.ReceiveFrom(Buffer, BufferSize, Received, FromAddress);
            ...
            if (Cb) Cb(Err, Received, FromAddress);
        });
    }

    void Helianthus::Network::Asio::AsyncUdpSocket::AsyncSendToProactor(const char* Data, size_t Size, const Network::NetworkAddress& Address, UdpSendHandler Handler)
    {
        // Windows 下优先使用 Proactor（IOCP）
#ifdef _WIN32
        if (ProactorPtr)
...
// 统一接口实现
Helianthus::Network::NetworkError Helianthus::Network::Asio::AsyncUdpSocket::Connect(const Helianthus::Network::NetworkAddress& Address)
{
...
void Helianthus::Network::Asio::AsyncUdpSocket::AsyncReceive(char* Buffer, size_t BufferSize, AsyncReceiveHandler Handler,
                                 CancelToken Token, uint32_t TimeoutMs)
{
    if (!ReactorPtr)
    {
        if (Handler) Handler(Helianthus::Network::NetworkError::NOT_INITIALIZED, 0, Helianthus::Network::NetworkAddress{});
        return;
    }
    ...
    this->AsyncReceiveFrom(Buffer, BufferSize, [this, Handler, Token](Helianthus::Network::NetworkError Error, size_t Bytes, Helianthus::Network::NetworkAddress FromAddr) {
        // 检查是否被取消
        if (Token && IsCancelled(Token))
        {
            if (Handler) Handler(Helianthus::Network::NetworkError::OPERATION_CANCELLED, 0, Helianthus::Network::NetworkAddress{});
        }
        else
        {
            if (Handler) Handler(Error, Bytes, FromAddr);
        }
...
void Helianthus::Network::Asio::AsyncUdpSocket::AsyncSend(const char* Data, size_t Size, const Helianthus::Network::NetworkAddress& Address,
                              AsyncSendHandler Handler, CancelToken Token, uint32_t TimeoutMs)
{
    if (!ReactorPtr)
    {
        if (Handler) Handler(Helianthus::Network::NetworkError::NOT_INITIALIZED, 0);
        return;
    }
    ...
    this->AsyncSendToProactor(Data, Size, Address, [this, Handler, Token](Helianthus::Network::NetworkError Error, size_t Bytes) {
        // 检查是否被取消
        if (Token && IsCancelled(Token))
        {
            if (Handler) Handler(Helianthus::Network::NetworkError::OPERATION_CANCELLED, 0);
        }
        else
        {
...
void Helianthus::Network::Asio::AsyncUdpSocket::AsyncConnect(const Helianthus::Network::NetworkAddress& Address, AsyncConnectHandler Handler,
                                 CancelToken Token, uint32_t TimeoutMs)
{
    // UDP不需要连接，直接返回成功
    if (Handler) Handler(Helianthus::Network::NetworkError::NONE);
}
...
Helianthus::Network::NetworkAddress Helianthus::Network::Asio::AsyncUdpSocket::GetLocalAddress() const
{
    return Socket.GetLocalAddress();
}

Helianthus::Network::NetworkAddress Helianthus::Network::Asio::AsyncUdpSocket::GetRemoteAddress() const
{
    // UDP没有固定的远程地址
