...
public:
    ...
    TaskId
    PostDelayedWithCancel(std::function<void()> Task, int DelayMs, CancelToken Token = nullptr);
...
private:
    ...
    struct DelayedTask
    {
        ...
        DelayedTask(TaskId IdIn,
                    std::function<void()> TaskIn,
                    int64_t ExecuteTimeIn,
                    CancelToken TokenIn = nullptr)
            : Id(IdIn),
              Task(std::move(TaskIn)),
              ExecuteTime(ExecuteTimeIn),
