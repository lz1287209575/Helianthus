...
namespace Helianthus::Discovery
{
    ...
    public:
        ...
        // 健康检查注册
        DiscoveryResult RegisterHealthCheck(ServiceInstanceId InstanceId, const HealthCheckConfig& Config) override;
        DiscoveryResult UpdateHealthCheck(ServiceInstanceId InstanceId, const HealthCheckConfig& Config) override;
        ...
        std::unordered_map<ServiceInstanceId, HealthScore> PerformBatchHealthCheck(const std::vector<ServiceInstanceId>& InstanceIds) override;
        ...
        // 健康检查类型实现
        DiscoveryResult PerformTcpHealthCheck(ServiceInstanceId InstanceId, const Network::NetworkAddress& Address, uint32_t TimeoutMs) override;
        DiscoveryResult PerformHttpHealthCheck(ServiceInstanceId InstanceId, const std::string& Url, const std::string& ExpectedResponse, uint32_t TimeoutMs) override;
        DiscoveryResult PerformCustomHealthCheck(ServiceInstanceId InstanceId, std::function<bool()> HealthCheckFunction) override;
        DiscoveryResult PerformHeartbeatCheck(ServiceInstanceId InstanceId) override;
        DiscoveryResult PerformPingCheck(ServiceInstanceId InstanceId, const Network::NetworkAddress& Address, uint32_t TimeoutMs) override;

        // 健康阈值和配置
        void SetHealthThresholds(ServiceInstanceId InstanceId, uint32_t UnhealthyThreshold, uint32_t HealthyThreshold) override;
        void GetHealthThresholds(ServiceInstanceId InstanceId, uint32_t& OutUnhealthyThreshold, uint32_t& OutHealthyThreshold) const override;
        void SetDefaultHealthThresholds(uint32_t UnhealthyThreshold, uint32_t HealthyThreshold) override;
        ...
        // 熔断器集成
        void EnableCircuitBreaker(ServiceInstanceId InstanceId, uint32_t FailureThreshold, uint32_t RecoveryTimeMs) override;
        ...
        // 自定义健康检查提供者
        DiscoveryResult RegisterCustomHealthCheckProvider(HealthCheckType Type, std::function<HealthScore(ServiceInstanceId)> Provider) override;
        ...
        void SetHealthStateChangeCallback(std::function<void(ServiceInstanceId, ServiceState, ServiceState)> Callback) override;
        void SetHealthAlertCallback(std::function<void(ServiceInstanceId, HealthScore, const std::string&)> Callback) override;
        void RemoveAllCallbacks() override;

        // 调试和诊断
        std::string GetHealthCheckInfo(ServiceInstanceId InstanceId) const override;
        std::vector<std::string> GetHealthCheckLog(ServiceInstanceId InstanceId, uint32_t MaxEntries = 100) const override;
        ...
        // 高级健康指标
        void UpdateCustomHealthMetric(ServiceInstanceId InstanceId, const std::string& MetricName, float Value) override;
        float GetCustomHealthMetric(ServiceInstanceId InstanceId, const std::string& MetricName) const override;
        std::unordered_map<std::string, float> GetAllCustomHealthMetrics(ServiceInstanceId InstanceId) const override;
        ...
        void ScheduleHealthCheck(ServiceInstanceId InstanceId, Common::TimestampMs ScheduleTime) override;
    ...
    private:
        ...
        ServiceState DetermineServiceState(HealthScore Score, uint32_t ConsecutiveFailures, uint32_t ConsecutiveSuccesses) const;
        void UpdateHealthEntry(ServiceInstanceId InstanceId, bool CheckSucceeded, uint32_t ResponseTime);
        void NotifyHealthChange(ServiceInstanceId InstanceId, ServiceState OldState, ServiceState NewState);
        void NotifyHealthAlert(ServiceInstanceId InstanceId, HealthScore Score, const std::string& Message);
        ...
        bool ExecuteHttpCheck(const std::string& Url, const std::string& ExpectedResponse, uint32_t TimeoutMs);
    ...
    private:
        ...
        std::unordered_map<HealthCheckType, std::function<HealthScore(ServiceInstanceId)>> CustomProviders;
