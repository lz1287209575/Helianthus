...
public:
    ...
    DiscoveryResult RegisterService(const ServiceInstance& Instance,
                                    ServiceInstanceId& OutInstanceId) override;
    DiscoveryResult UpdateService(ServiceInstanceId InstanceId,
    ...
    GetServicesByName(const std::string& ServiceName) const override;
    std::vector<ServiceInstancePtr>
    GetHealthyServices(const std::string& ServiceName) const override;
    ...
    FindServices(const std::string& ServiceName,
                 ...
                 ServiceState MinState = ServiceState::HEALTHY) const override;

    std::vector<ServiceInstancePtr>
    FindServicesByTag(const std::string& TagKey, const std::string& TagValue = "") const override;
    ...
    DiscoveryResult UpdateServiceLoad(ServiceInstanceId InstanceId,
                                      uint32_t ActiveConnections) override;
    ...
    DiscoveryResult CreateServiceGroup(const std::string& ServiceName,
                                       const LoadBalanceConfig& Config,
                                       ServiceGroupId& OutGroupId) override;
    DiscoveryResult UpdateServiceGroup(ServiceGroupId GroupId,
                                       const LoadBalanceConfig& Config) override;
    ...
    DiscoveryResult
    EnableReplication(const std::vector<Network::NetworkAddress>& ReplicaNodes) override;
...
private:
    ...
    void
    NotifyStateChange(ServiceInstanceId InstanceId, ServiceState OldState, ServiceState NewState);
