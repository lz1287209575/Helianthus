...
struct ServiceEndpoint
{
    ...
    bool IsValid() const
    {
        return Address.IsValid() && !Protocol.empty();
    }
    std::string ToString() const
    {
        return Protocol + "://" + Address.ToString();
    }
};
...
struct ServiceGroup
{
    ...
    size_t GetHealthyInstanceCount() const
    {
...
using ServiceStateChangeCallback =
    std::function<void(ServiceInstanceId InstanceId, ServiceState OldState, ServiceState NewState)>;
using ServiceRegistrationCallback =
    std::function<void(ServiceInstanceId InstanceId, DiscoveryResult Result)>;
using ServiceDiscoveryCallback = std::function<void(
    const std::string& ServiceName, const std::vector<ServiceInstancePtr>& Instances)>;
using HealthCheckCallback =
    std::function<void(ServiceInstanceId InstanceId, bool IsHealthy, HealthScore Score)>;
using LoadBalanceCallback =
    std::function<void(ServiceInstanceId SelectedInstanceId, const std::string& Reason)>;
