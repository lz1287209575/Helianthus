...
public:
    ...
    virtual DiscoveryResult RegisterService(const ServiceInstance& Instance,
                                            ServiceInstanceId& OutInstanceId) = 0;
    virtual DiscoveryResult UpdateService(ServiceInstanceId InstanceId,
    ...
    GetServicesByName(const std::string& ServiceName) const = 0;
    virtual std::vector<ServiceInstancePtr>
    GetHealthyServices(const std::string& ServiceName) const = 0;
    ...
    virtual std::vector<ServiceInstancePtr>
    FindServices(const std::string& ServiceName,
                 const std::unordered_map<std::string, std::string>& Tags = {},
                 const std::string& Region = "",
                 const std::string& Zone = "",
                 ServiceState MinState = ServiceState::HEALTHY) const = 0;

    virtual std::vector<ServiceInstancePtr>
    FindServicesByTag(const std::string& TagKey, const std::string& TagValue = "") const = 0;

    virtual std::vector<ServiceInstancePtr>
    FindServicesByRegion(const std::string& Region) const = 0;
    virtual std::vector<ServiceInstancePtr> FindServicesByZone(const std::string& Zone) const = 0;

    // Service state management
    virtual DiscoveryResult UpdateServiceState(ServiceInstanceId InstanceId,
                                               ServiceState State) = 0;
    virtual DiscoveryResult UpdateServiceHealth(ServiceInstanceId InstanceId,
                                                HealthScore Score) = 0;
    virtual DiscoveryResult UpdateServiceLoad(ServiceInstanceId InstanceId,
    ...
    virtual DiscoveryResult CreateServiceGroup(const std::string& ServiceName,
                                               const LoadBalanceConfig& Config,
                                               ServiceGroupId& OutGroupId) = 0;
    virtual DiscoveryResult UpdateServiceGroup(ServiceGroupId GroupId,
                                               const LoadBalanceConfig& Config) = 0;
    ...
    EnableReplication(const std::vector<Network::NetworkAddress>& ReplicaNodes) = 0;
