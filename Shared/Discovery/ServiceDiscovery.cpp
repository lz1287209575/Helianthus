                                             ...
                                             const LoadBalanceConfig& LoadBalanceConfig)
{
    ...
    Registry->SetServiceRegistrationCallback([this](ServiceInstanceId Id, DiscoveryResult Result)
    ...
    LoadBalance->SetLoadBalanceCallback([this](ServiceInstanceId Id, const std::string& ServiceName)
...
ServiceDiscovery::DiscoverHealthyServices(const std::string& ServiceName)
{
    if (!InitializedFlag)
    {
        return {};
...
DiscoveryResult
ServiceDiscovery::ConnectToService(const std::string& ServiceName,
                                   ...
                                   std::function<void(const std::string&)> OnError)
{
    ...
    {
        if (OnError)
            OnError("ServiceDiscovery not initialized");
        return DiscoveryResult::INTERNAL_ERROR;
    }
    ...
    {
        if (OnError)
            OnError("No available service instance found");
...
void ServiceDiscovery::OnServiceStateChanged(ServiceInstanceId InstanceId,
...
void ServiceDiscovery::OnHealthAlert(ServiceInstanceId InstanceId,
                                     HealthScore Score,
...
void ServiceDiscovery::OnLoadBalanceEvent(ServiceInstanceId InstanceId,
...
DiscoveryResult
ServiceDiscovery::CreateRpcConnection(ServiceInstancePtr Instance,
                                      std::function<void(ServiceInstancePtr)> OnConnected,
                                      std::function<void(const std::string&)> OnError)
{
    if (!Instance)
    {
        if (OnError)
            OnError("Invalid service instance");
        return DiscoveryResult::INVALID_SERVICE_INFO;
    }
...
DiscoveryResult
ServiceDiscovery::EnableReplication(const std::vector<Network::NetworkAddress>& ReplicaNodes)
{
    if (Registry)
    {
                                        ...
                                        const LoadBalanceConfig& LoadBalanceConfig)
{
    ...
    return Discovery.Initialize(RegistryConfig, HealthConfig, LoadBalanceConfig) ==
