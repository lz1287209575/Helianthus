...
public:
    ...
    virtual DiscoveryResult RegisterHealthCheck(ServiceInstanceId InstanceId,
                                                const HealthCheckConfig& Config) = 0;
    virtual DiscoveryResult UpdateHealthCheck(ServiceInstanceId InstanceId,
                                              const HealthCheckConfig& Config) = 0;
    ...
    virtual std::unordered_map<ServiceInstanceId, HealthScore>
    PerformBatchHealthCheck(const std::vector<ServiceInstanceId>& InstanceIds) = 0;
    ...
    virtual DiscoveryResult PerformTcpHealthCheck(ServiceInstanceId InstanceId,
                                                  const Network::NetworkAddress& Address,
                                                  uint32_t TimeoutMs) = 0;
    virtual DiscoveryResult PerformHttpHealthCheck(ServiceInstanceId InstanceId,
                                                   const std::string& Url,
                                                   const std::string& ExpectedResponse,
                                                   uint32_t TimeoutMs) = 0;
    virtual DiscoveryResult PerformCustomHealthCheck(ServiceInstanceId InstanceId,
                                                     std::function<bool()> HealthCheckFunction) = 0;
    virtual DiscoveryResult PerformHeartbeatCheck(ServiceInstanceId InstanceId) = 0;
    virtual DiscoveryResult PerformPingCheck(ServiceInstanceId InstanceId,
                                             const Network::NetworkAddress& Address,
                                             uint32_t TimeoutMs) = 0;

    // Health thresholds and configuration
    virtual void SetHealthThresholds(ServiceInstanceId InstanceId,
                                     uint32_t UnhealthyThreshold,
                                     uint32_t HealthyThreshold) = 0;
    virtual void GetHealthThresholds(ServiceInstanceId InstanceId,
                                     uint32_t& OutUnhealthyThreshold,
                                     uint32_t& OutHealthyThreshold) const = 0;
    virtual void SetDefaultHealthThresholds(uint32_t UnhealthyThreshold,
                                            uint32_t HealthyThreshold) = 0;
    ...
    virtual void EnableCircuitBreaker(ServiceInstanceId InstanceId,
    ...
    RegisterCustomHealthCheckProvider(HealthCheckType Type,
                                      std::function<HealthScore(ServiceInstanceId)> Provider) = 0;
    ...
    virtual void SetHealthStateChangeCallback(
        std::function<void(ServiceInstanceId, ServiceState, ServiceState)> Callback) = 0;
    virtual void SetHealthAlertCallback(
        std::function<void(ServiceInstanceId, HealthScore, const std::string&)> Callback) = 0;
    virtual void RemoveAllCallbacks() = 0;

    // Debugging and diagnostics
    virtual std::string GetHealthCheckInfo(ServiceInstanceId InstanceId) const = 0;
    virtual std::vector<std::string> GetHealthCheckLog(ServiceInstanceId InstanceId,
                                                       uint32_t MaxEntries = 100) const = 0;
    ...
    virtual void UpdateCustomHealthMetric(ServiceInstanceId InstanceId,
                                          const std::string& MetricName,
                                          float Value) = 0;
    virtual float GetCustomHealthMetric(ServiceInstanceId InstanceId,
                                        const std::string& MetricName) const = 0;
    virtual std::unordered_map<std::string, float>
    GetAllCustomHealthMetrics(ServiceInstanceId InstanceId) const = 0;
    ...
    virtual void ScheduleHealthCheck(ServiceInstanceId InstanceId,
                                     Common::TimestampMs ScheduleTime) = 0;
