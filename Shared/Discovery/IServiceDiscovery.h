...
namespace Helianthus::Discovery
{
    ...
    public:
        ...
        virtual ServiceInstancePtr DiscoverServiceByTag(const std::string& ServiceName, const std::unordered_map<std::string, std::string>& Tags) = 0;
        virtual ServiceInstancePtr DiscoverServiceByRegion(const std::string& ServiceName, const std::string& Region) = 0;
        virtual ServiceInstancePtr DiscoverServiceByZone(const std::string& ServiceName, const std::string& Zone) = 0;

        // Asynchronous discovery
        virtual std::future<ServiceInstancePtr> DiscoverServiceAsync(const std::string& ServiceName) = 0;
        ...
        virtual void AddServiceFilter(const std::string& ServiceName, const std::unordered_map<std::string, std::string>& RequiredTags) = 0;
        ...
        virtual void SetConnectionCallback(std::function<void(Network::ConnectionId, bool)> Callback) = 0;
        virtual void RemoveAllCallbacks() = 0;

        // Circuit breaker integration
        virtual void EnableCircuitBreaker(const std::string& ServiceName, uint32_t FailureThreshold = 5, uint32_t RecoveryTimeMs = 30000) = 0;
