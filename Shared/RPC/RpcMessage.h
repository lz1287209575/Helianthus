...
namespace Helianthus::RPC
{
    ...
    public:
        ...
        // Accessors
        RpcContext GetContext() const { return Payload.Context; }
        void SetContext(const RpcContext& Context) { Payload.Context = Context; }

        const std::string& GetParameters() const { return Payload.Parameters; }
        void SetParameters(const std::string& Parameters) { Payload.Parameters = Parameters; }

        const std::string& GetResult() const { return Payload.Result; }
        void SetResult(const std::string& Result) { Payload.Result = Result; }

        RpcResult GetErrorCode() const { return Payload.ErrorCode; }
        void SetError(RpcResult Code, const std::string& Message);

        // Type checking
        bool IsRequest() const { return Payload.Context.CallType == RpcCallType::REQUEST; }
        bool IsResponse() const { return Payload.Context.CallType == RpcCallType::RESPONSE; }
        bool IsNotification() const { return Payload.Context.CallType == RpcCallType::NOTIFICATION; }
        bool IsError() const { return Payload.ErrorCode != RpcResult::SUCCESS; }
