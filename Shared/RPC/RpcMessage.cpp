...
namespace Helianthus::RPC
{
    ...
    bool RpcMessagePayload::FromBinary(const std::vector<uint8_t>& Data)
    {
        ...
        if (!ReadString(Context.ServiceName) ||
            !ReadString(Context.MethodName) ||
            !ReadString(Parameters) ||
            !ReadString(Result) ||
            !ReadString(ErrorMessage))
        {
            return false;
        }
    ...
    Message::MessagePtr RpcMessage::ToMessage() const
    {
        ...
        switch (Payload.Context.Format)
        {
            ...
            case SerializationFormat::BINARY:
            {
                ...
                SerializedPayload.assign(reinterpret_cast<const char*>(BinaryData.data()), BinaryData.size());
                break;
            }
            default:
                SerializedPayload = Payload.ToJson();
        ...
        Msg->SetSenderId(static_cast<Common::ServerId>(std::hash<std::string>{}(Payload.Context.ClientId)));
        Msg->SetReceiverId(static_cast<Common::ServerId>(std::hash<std::string>{}(Payload.Context.ServerId)));
