...
namespace Helianthus::RPC
{
    ...
    Network::NetworkError NetworkAdapter::StartServer(const Network::NetworkAddress& Address, Network::ProtocolType Protocol)
    {
        if (!IsInitialized)
        {
    ...
    Network::ConnectionId NetworkAdapter::ConnectToServer(const Network::NetworkAddress& Address, Network::ProtocolType Protocol)
    {
        if (!IsInitialized)
        {
    ...
    void NetworkAdapter::SetOnClientConnectedCallback(std::function<void(Network::ConnectionId)> Callback)
    {
        ClientConnectedCallback = Callback;
    }

    void NetworkAdapter::SetOnClientDisconnectedCallback(std::function<void(Network::ConnectionId, Network::NetworkError)> Callback)
    {
