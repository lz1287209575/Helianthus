...
RpcResult RpcServer::Start(const Network::NetworkAddress& BindAddress)
{
    ...
    // Start server
    Network::NetworkError StartResult =
        ImplPtr->NetworkManager->StartServer(BindAddress, Network::ProtocolType::TCP);
    ...
    ImplPtr->NetworkManager->SetOnClientConnectedCallback(
        [this](Network::ConnectionId ConnId)
        {
    ...
    ImplPtr->NetworkManager->SetOnClientDisconnectedCallback(
        [this](Network::ConnectionId ConnId, Network::NetworkError Error)
        {
            std::string ClientId;
    ...
    ImplPtr->NetworkManager->SetOnDataReceivedCallback(
        [this](Network::ConnectionId ConnId, const char* Data, size_t Size)
        {
            std::string ClientId;
    ...
    {
        ImplPtr->WorkerThreads.emplace_back(
            [this]()
            {
...
void RpcServer::SendResponse(const std::string& ClientId, const RpcMessage& Response)
{
    ...
    ImplPtr->NetworkManager->SendToClient(
        ConnId, reinterpret_cast<const char*>(Payload.data()), Payload.size());
}

RpcResult RpcServer::ProcessRequest(const RpcContext& Context,
...
void RpcServer::UpdateStats(const RpcContext& Context, RpcResult Result, uint64_t LatencyMs)
{
    ...
    else
    {
        ...
        uint64_t TotalLatency =
            ImplPtr->GlobalStats.AverageLatencyMs * (ImplPtr->GlobalStats.TotalCalls - 1) +
            LatencyMs;
...
void RpcServiceBase::RegisterAsyncMethod(
    const std::string& MethodName,
