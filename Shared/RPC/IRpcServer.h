...
namespace Helianthus::RPC
{
    ...
    public:
        ...
        // Service lifecycle
        virtual RpcResult Initialize() { return RpcResult::SUCCESS; }
    ...
    public:
        ...
        std::string GetServiceName() const override { return ServiceName; }
        std::string GetServiceVersion() const override { return ServiceVersion; }
    ...
    protected:
        ...
        void RegisterAsyncMethod(const std::string& MethodName, 
                               std::function<void(const RpcContext&, const std::string&, RpcCallback)> Handler);

        // Utility methods
        void SetServiceVersion(const std::string& Version) { ServiceVersion = Version; }
        ...
        template<typename TRequest, typename TResponse>
        void RegisterTypedAsyncMethod(const std::string& MethodName,
    ...
    private:
        ...
        std::unordered_map<std::string, std::function<void(const RpcContext&, const std::string&, RpcCallback)>> AsyncMethods;
    };

    // Template implementations
    template<typename TRequest, typename TResponse>
    void RpcServiceBase::RegisterTypedMethod(const std::string& MethodName,
                                           std::function<RpcResult(const TRequest&, TResponse&)> Handler)
    {
    ...
    template<typename TRequest, typename TResponse>
    void RpcServiceBase::RegisterTypedAsyncMethod(const std::string& MethodName,
                                                std::function<void(const TRequest&, std::function<void(RpcResult, const TResponse&)>)> Handler)
    {
        auto Wrapper = [Handler](const RpcContext& Context, const std::string& Parameters, RpcCallback Callback) {
            TRequest Request; // TODO: Deserialize Parameters to Request
