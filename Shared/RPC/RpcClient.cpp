...
RpcClient::RpcClient(const RpcConfig& Config) : ImplPtr(std::make_unique<Impl>())
{
    ...
    // Start worker thread
    ImplPtr->WorkerThread = std::thread(
        [this]()
        {
...
RpcResult RpcClient::Connect(const Network::NetworkAddress& ServerAddress)
{
    ...
    ImplPtr->ConnectionId =
        ImplPtr->NetworkManager->ConnectToServer(ServerAddress, Network::ProtocolType::TCP);
...
RpcFuture<std::pair<RpcResult, std::string>> RpcClient::CallFuture(const std::string& ServiceName,
                                                                   const std::string& MethodName,
                                                                   const std::string& Parameters,
                                                                   uint32_t TimeoutMs)
{
...
RpcResult RpcClient::SendRequest(const RpcMessage& Request, uint32_t TimeoutMs)
{
    ...
    Network::NetworkError SendResult = ImplPtr->NetworkManager->SendToClient(
        ImplPtr->ConnectionId, reinterpret_cast<const char*>(Payload.data()), Payload.size());
