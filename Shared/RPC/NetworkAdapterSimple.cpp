...
Network::NetworkError NetworkAdapterSimple::StartServer(const Network::NetworkAddress& Address,
...
Network::ConnectionId NetworkAdapterSimple::ConnectToServer(const Network::NetworkAddress& Address,
                                                            Network::ProtocolType Protocol)
{
    if (!IsInitialized)
    {
...
NetworkAdapterSimple::SendToClient(Network::ConnectionId ClientId, const char* Data, size_t Size)
{
...
void NetworkAdapterSimple::SetOnClientConnectedCallback(
    std::function<void(Network::ConnectionId)> Callback)
{
    ClientConnectedCallback = Callback;
}

void NetworkAdapterSimple::SetOnClientDisconnectedCallback(
    std::function<void(Network::ConnectionId, Network::NetworkError)> Callback)
{
    ClientDisconnectedCallback = Callback;
}

void NetworkAdapterSimple::SetOnDataReceivedCallback(
    std::function<void(Network::ConnectionId, const char*, size_t)> Callback)
{
    DataReceivedCallback = Callback;
}

void NetworkAdapterSimple::HandleIncomingMessage(Network::ConnectionId ConnId,
                                                 const char* Data,
                                                 size_t Size)
{
    if (DataReceivedCallback)
    {
        std::cout << "[NetworkAdapterSimple] 接收消息，连接: " << ConnId << ", 大小: " << Size
                  << " 字节" << std::endl;
