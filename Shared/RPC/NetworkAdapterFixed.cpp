...
namespace Helianthus::RPC
{
    ...
    Network::ConnectionId NetworkAdapterFixed::ConnectToServer(const Network::NetworkAddress& Address, Network::ProtocolType Protocol)
    {
        if (!IsInitialized)
        {
        ...
        std::cout << "[NetworkAdapter] 客户端连接到 " << Address.ToString() 
        ...
        std::thread([this, ConnId, Address]() {
    ...
    Network::NetworkError NetworkAdapterFixed::SendToClient(Network::ConnectionId ClientId, const uint8_t* Data, size_t Size)
    {
        ...
        std::cout << "[NetworkAdapter] 发送消息到连接 " << ClientId 
                  << ", 大小: " << Size << " 字节" << std::endl;
        ...
        if (IsServerConnection)
        {
            ...
            std::thread([this, Message]() {
                ...
                {
                    std::cout << "[NetworkAdapter] 服务器响应传递到客户端，连接: " << Message.ConnectionId 
    ...
    void NetworkAdapterFixed::SetOnClientConnectedCallback(std::function<void(Network::ConnectionId)> Callback)
    {
        ClientConnectedCallback = Callback;
    }

    void NetworkAdapterFixed::SetOnClientDisconnectedCallback(std::function<void(Network::ConnectionId, Network::NetworkError)> Callback)
    {
