...
namespace Helianthus::Reflection
{
    ...
    inline void RegisterProperty(const std::string& ClassName, const std::string& PropertyName, const std::string& PropertyType)
    {
        if (GHelianthusReflectionSystem)
        {
    ...
    inline void RegisterFunction(const std::string& ClassName, const std::string& FunctionName, const std::string& ReturnType)
    {
        if (GHelianthusReflectionSystem)
        {
...
// 简化的宏定义
#define HCLASS(ClassName, BaseClass) \
    class ClassName : public BaseClass

#define HPROPERTY(PropertyName, PropertyType) \
    PropertyType PropertyName;

#define HFUNCTION(FunctionName, ReturnType, ...) \
    ReturnType FunctionName(__VA_ARGS__)

#define HENUM(EnumName) \
    enum class EnumName : int32_t

// 自动注册宏 - 使用命名空间避免语法错误
#define REGISTER_CLASS(ClassName) \
    namespace AutoRegister##ClassName { \
        static bool Registered = (Helianthus::Reflection::RegisterClass(#ClassName, #ClassName), true); \
    }

#define REGISTER_PROPERTY(ClassName, PropertyName, PropertyType) \
    namespace AutoRegister##ClassName##_##PropertyName { \
        static bool Registered = (Helianthus::Reflection::RegisterProperty(#ClassName, #PropertyName, #PropertyType), true); \
    }

#define REGISTER_FUNCTION(ClassName, FunctionName, ReturnType, ...) \
    namespace AutoRegister##ClassName##_##FunctionName { \
        static bool Registered = (Helianthus::Reflection::RegisterFunction(#ClassName, #FunctionName, #ReturnType), true); \
    }
