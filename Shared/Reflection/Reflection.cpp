...
void HelianthusReflectionSystem::RegisterHProperty(const std::string& ClassName,
                                                   const HPropertyInfo& PropertyInfo)
{
    HProperties[ClassName][PropertyInfo.PropertyName] = PropertyInfo;
    std::cout << "注册属性: " << ClassName << "::" << PropertyInfo.PropertyName << std::endl;
}

void HelianthusReflectionSystem::RegisterHFunction(const std::string& ClassName,
                                                   const HFunctionInfo& FunctionInfo)
{
...
const HPropertyInfo*
HelianthusReflectionSystem::GetHPropertyInfo(const std::string& ClassName,
...
const HFunctionInfo*
HelianthusReflectionSystem::GetHFunctionInfo(const std::string& ClassName,
...
void HelianthusReflectionSystem::SetHProperty(void* Object,
...
void* HelianthusReflectionSystem::CallHFunction(void* Object,
...
bool HelianthusReflectionSystem::IsHProperty(const std::string& ClassName,
                                             const std::string& PropertyName) const
{
    auto ClassIt = HProperties.find(ClassName);
    if (ClassIt == HProperties.end())
...
bool HelianthusReflectionSystem::IsHFunction(const std::string& ClassName,
                                             const std::string& FunctionName) const
{
    auto ClassIt = HFunctions.find(ClassName);
    if (ClassIt == HFunctions.end())
...
HelianthusReflectionSystem::GetAllHPropertyNames(const std::string& ClassName) const
{
    std::vector<std::string> Names;
    auto ClassIt = HProperties.find(ClassName);
...
std::vector<std::string>
HelianthusReflectionSystem::GetAllHFunctionNames(const std::string& ClassName) const
{
    std::vector<std::string> Names;
    auto ClassIt = HFunctions.find(ClassName);
...
std::string HelianthusReflectionSystem::GenerateHPropertyCode(const std::string& ClassName,
                                                              const std::string& PropertyName) const
{
...
std::string HelianthusReflectionSystem::GenerateHFunctionCode(const std::string& ClassName,
                                                              const std::string& FunctionName) const
{
    ...
    for (size_t i = 0; i < FunctionInfo->Parameters.size(); ++i)
    {
        if (i > 0)
            Code += ", ";
        Code += FunctionInfo->Parameters[i].ParameterType + " " +
                FunctionInfo->Parameters[i].ParameterName;
    }
...
bool HelianthusReflectionSystem::SaveScriptBindings(const std::string& FilePath,
