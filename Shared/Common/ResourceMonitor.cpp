...
void ResourceMonitor::CleanupOldHistory()
{
    ...
    HistoryStats.erase(std::remove_if(HistoryStats.begin(),
                                      HistoryStats.end(),
...
void ResourceMonitor::CollectCpuStatsImpl(ResourceUsageStats& Stats)
{
            ...
            if (LastTotal != 0)
            {
                ...
                if (TotalDiff > 0)
                {
                    Stats.CpuUsagePercent =
                        100.0 - (static_cast<double>(IdleDiff) / TotalDiff * 100.0);
                }
            }
    ...
    {
        LoadAvgFile >> Stats.CpuLoadAverage1Min >> Stats.CpuLoadAverage5Min >>
            Stats.CpuLoadAverage15Min;
        LoadAvgFile.close();
    }
    ...
    if (host_statistics(Host, HOST_CPU_LOAD_INFO, (host_info_t)&CpuLoad, &Count) == KERN_SUCCESS)
    {
        ...
        if (LastUser != 0)
        {
            ...
            {
                Stats.CpuUsagePercent =
                    static_cast<double>(UserDiff + SystemDiff) / TotalDiff * 100.0;
            }
        }
    ...
    if (sysctl(Mib, 2, &LoadAvg, &Size, nullptr, 0) == 0)
    {
        ...
        if (Scale == 0)
            Scale = 1;
        Stats.CpuLoadAverage1Min =
            static_cast<double>(LoadAvg.ldavg[0]) / static_cast<double>(Scale);
        Stats.CpuLoadAverage5Min =
            static_cast<double>(LoadAvg.ldavg[1]) / static_cast<double>(Scale);
        Stats.CpuLoadAverage15Min =
            static_cast<double>(LoadAvg.ldavg[2]) / static_cast<double>(Scale);
    }
...
void ResourceMonitor::CollectMemoryStatsImpl(ResourceUsageStats& Stats)
{
    ...
    if (GlobalMemoryStatusEx(&MemStatus))
    {
        ...
        Stats.SwapUsagePercent =
            Stats.SwapTotalBytes > 0
                ? (static_cast<double>(Stats.SwapUsedBytes) / Stats.SwapTotalBytes) * 100.0
                : 0.0;
    }
    ...
    if (MemInfoFile.is_open())
    {
        ...
        while (std::getline(MemInfoFile, Line))
        {
            ...
            if (Key == "MemTotal:")
                MemTotal = std::stoull(Value) * 1024;
            else if (Key == "MemAvailable:")
                MemAvailable = std::stoull(Value) * 1024;
            else if (Key == "MemFree:")
                MemFree = std::stoull(Value) * 1024;
            else if (Key == "Buffers:")
                Buffers = std::stoull(Value) * 1024;
            else if (Key == "Cached:")
                Cached = std::stoull(Value) * 1024;
            else if (Key == "SwapTotal:")
                SwapTotal = std::stoull(Value) * 1024;
            else if (Key == "SwapFree:")
                SwapFree = std::stoull(Value) * 1024;
        }
    ...
    if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info64_t)&VmStats, &Count) ==
        KERN_SUCCESS)
    {
        uint64_t TotalMemory = (VmStats.active_count + VmStats.inactive_count + VmStats.wire_count +
                                VmStats.free_count) *
                               PageSize;
    ...
    if (GetDiskFreeSpaceExA(MountPoint.c_str(), &FreeBytesAvailable, &TotalBytes, &TotalFreeBytes))
    {
        ...
        DiskStats.UsagePercent =
            DiskStats.TotalBytes > 0
                ? (static_cast<double>(DiskStats.UsedBytes) / DiskStats.TotalBytes) * 100.0
                : 0.0;
...
void CollectAllNetworkStats(ResourceUsageStats& Stats)
{
    ...
    if (NetDevFile.is_open())
    {
        ...
        while (std::getline(NetDevFile, Line))
        {
            ...
            Iss >> InterfaceName >> BytesReceived >> PacketsReceived >> ErrorsReceived >>
                DroppedReceived >> BytesSent >> PacketsSent >> ErrorsSent >> DroppedSent;
