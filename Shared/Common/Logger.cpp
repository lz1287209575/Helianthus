...
namespace Helianthus::Common
{
    ...
    void Logger::Initialize(const LoggerConfig& Config)
    {
        ...
        {
            Sinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>(Config.FilePath, Config.MaxFileSize, Config.MaxFiles));
        }
    ...
    {
        if (LoggerInstance) LoggerInstance->debug("{}", Message);
    }

    void Logger::Info(const std::string& Message)
    {
        if (LoggerInstance) LoggerInstance->info("{}", Message);
    }

    void Logger::Warn(const std::string& Message)
    {
        if (LoggerInstance) LoggerInstance->warn("{}", Message);
    }

    void Logger::Error(const std::string& Message)
    {
        if (LoggerInstance) LoggerInstance->error("{}", Message);
    }

    void Logger::SetLevel(LogLevel Level)
    {
        if (LoggerInstance) LoggerInstance->set_level(ConvertLogLevel(Level));
    }

    LogLevel Logger::GetLevel()
    {
        if (LoggerInstance) return ConvertLogLevel(LoggerInstance->level());
        return CurrentConfig.Level;
    }

    void Logger::Flush()
    {
        if (LoggerInstance) LoggerInstance->flush();
    }
}
