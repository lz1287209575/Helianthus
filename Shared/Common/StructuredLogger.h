...
{
// 日志字段值类型
using LogFieldValue = std::variant<std::string, int32_t, int64_t, uint32_t, uint64_t, double, bool>;
...
public:
    ...
    // 获取所有字段
    const std::unordered_map<std::string, LogFieldValue>& GetAllFields() const
    {
        return Fields;
    }
...
public:
    ...
    // 便捷方法
    static void Trace(const std::string& Category,
                      const std::string& Message,
                      const LogFields& Fields = LogFields());
    static void Debug(const std::string& Category,
                      const std::string& Message,
                      const LogFields& Fields = LogFields());
    static void Info(const std::string& Category,
                     const std::string& Message,
                     const LogFields& Fields = LogFields());
    static void Warn(const std::string& Category,
                     const std::string& Message,
                     const LogFields& Fields = LogFields());
    static void Error(const std::string& Category,
                      const std::string& Message,
                      const LogFields& Fields = LogFields());
    static void Fatal(const std::string& Category,
                      const std::string& Message,
                      const LogFields& Fields = LogFields());
    ...
    // 性能指标
    static void
    RecordMetric(const std::string& Name, double Value, const LogFields& Tags = LogFields());
    static void IncrementCounter(const std::string& Name,
                                 int64_t Value = 1,
                                 const LogFields& Tags = LogFields());
    static void
    RecordHistogram(const std::string& Name, double Value, const LogFields& Tags = LogFields());
...
{
public:
    LogScope(const std::string& Category,
