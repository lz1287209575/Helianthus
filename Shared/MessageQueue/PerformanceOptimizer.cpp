...
void PerformanceOptimizer::DeallocateToPool(void* Ptr, size_t Size)
{
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Verbose, 
          "释放到{}: ptr={}, size={}", ReturnedToPool ? "内存池" : "系统", Ptr, Size);
}
...
MessagePtr PerformanceOptimizer::CreateMessage(MessageType Type)
{
    ...
    
    {
        ...
        if (!MessagePool.empty())
        {
            ...
            H_LOG(MQ, Helianthus::Common::LogVerbosity::Verbose, 
                  "从对象池创建消息: type={}, id={}", 
                  static_cast<int>(Type), static_cast<uint64_t>(Result->Header.Id));
        }
    }
...
void PerformanceOptimizer::RecycleMessage(MessagePtr Message)
{
    ...
    if (MessagePool.size() < Config.MessagePoolMaxSize)
    {
        ...
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Verbose, 
              "回收消息对象到池: id={}", static_cast<uint64_t>(Message->Header.Id));
    }
}
...
MessagePtr PerformanceOptimizer::CreateMessageFromZeroCopy(const ZeroCopyBuffer& Buffer, MessageType Type)
{
...
uint32_t PerformanceOptimizer::CreateBatch(const std::string& QueueName)
{
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, 
          "创建批处理: id={}, queue={}", BatchId, QueueName);
...
bool PerformanceOptimizer::AddToBatch(uint32_t BatchId, MessagePtr Message)
{
    ...
    size_t currentCount = 0;
    {
        ...
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Verbose, 
              "添加到批处理: batch_id={}, count={}", BatchId, currentCount);
...
bool PerformanceOptimizer::CommitBatch(uint32_t BatchId)
{
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, 
          "提交批处理: id={}, messages={}", BatchId, Messages.size());
...
bool PerformanceOptimizer::ResetBatch(uint32_t BatchId, const std::string& QueueName)
{
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display,
          "重置批处理: id={}, queue={}", BatchId, Batch.QueueName);
...
void PerformanceOptimizer::InitializeMemoryPool()
{
    ...
    if (!MemoryPoolData)
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Error, 
              "内存池初始化失败: 分配 {} 字节失败", MemoryPoolSize);
        return;
    }
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, 
          "内存池初始化完成: size={}, blocks={}", MemoryPoolSize, BlockCount);
}
...
void PerformanceOptimizer::InitializeMessagePool()
{
    ...
    if (Config.EnablePreallocation)
    {
        ...
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, 
              "消息对象池预分配完成: count={}", Config.PreallocatedMessages);
    }
}
...
void PerformanceOptimizer::UpdatePerformanceMetrics()
{
    ...
    {
        Stats.MemoryPoolHitRate = static_cast<double>(Stats.MemoryPoolHits) / Stats.TotalAllocations * 100.0;
    }
