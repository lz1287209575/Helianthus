...
public:
    ...
    // 队列持久化
    virtual QueueResult
    SaveQueue(const std::string& QueueName, const QueueConfig& Config, const QueueStats& Stats) = 0;
    virtual QueueResult
    LoadQueue(const std::string& QueueName, QueueConfig& Config, QueueStats& Stats) = 0;
    ...
    virtual QueueResult
    LoadMessage(const std::string& QueueName, MessageId MessageId, MessagePtr& OutMessage) = 0;
    virtual QueueResult DeleteMessage(const std::string& QueueName, MessageId MessageId) = 0;
    virtual QueueResult SaveBatchMessages(const std::string& QueueName,
                                          const std::vector<MessagePtr>& Messages) = 0;
    virtual QueueResult LoadAllMessages(const std::string& QueueName,
                                        std::vector<MessagePtr>& OutMessages) = 0;
    ...
    struct PersistenceStats
    {
        ...
        {
            return TotalWriteCount > 0 ? static_cast<double>(TotalWriteTimeMs) / TotalWriteCount
                                       : 0.0;
        }
...
public:
    ...
    QueueResult SaveQueue(const std::string& QueueName,
                          const QueueConfig& Config,
                          const QueueStats& Stats) override;
    QueueResult
    LoadQueue(const std::string& QueueName, QueueConfig& Config, QueueStats& Stats) override;
    ...
    QueueResult
    LoadMessage(const std::string& QueueName, MessageId MessageId, MessagePtr& OutMessage) override;
    QueueResult DeleteMessage(const std::string& QueueName, MessageId MessageId) override;
    QueueResult SaveBatchMessages(const std::string& QueueName,
                                  const std::vector<MessagePtr>& Messages) override;
    QueueResult LoadAllMessages(const std::string& QueueName,
                                std::vector<MessagePtr>& OutMessages) override;
...
private:
    ...
    // 索引缓存
    std::unordered_map<std::string, std::unordered_map<MessageId, MessageIndexEntry>>
        QueueMessageIndex;
    ...
    struct PersistenceMetrics
    {
        ...
        void RecordWriteTime(uint64_t TimeMs)
        {
            ...
            while (TimeMs > CurrentMax && !MaxWriteTimeMs.compare_exchange_weak(
                                              CurrentMax, TimeMs, std::memory_order_relaxed))
                ;

            uint64_t CurrentMin = MinWriteTimeMs.load(std::memory_order_relaxed);
            while (TimeMs < CurrentMin && !MinWriteTimeMs.compare_exchange_weak(
                                              CurrentMin, TimeMs, std::memory_order_relaxed))
                ;
        }
        ...
        void RecordReadTime(uint64_t TimeMs)
        {
            ...
            while (TimeMs > CurrentMax && !MaxReadTimeMs.compare_exchange_weak(
                                              CurrentMax, TimeMs, std::memory_order_relaxed))
                ;

            uint64_t CurrentMin = MinReadTimeMs.load(std::memory_order_relaxed);
            while (TimeMs < CurrentMin && !MinReadTimeMs.compare_exchange_weak(
                                              CurrentMin, TimeMs, std::memory_order_relaxed))
                ;
        }
    ...
    QueueResult UpdateMessageIndex(const std::string& QueueName,
                                   MessageId MessageId,
                                   size_t FileOffset,
                                   size_t MessageSize);
...
public:
    ...
    // 委托给具体的持久化实现
    QueueResult
    SaveQueue(const std::string& QueueName, const QueueConfig& Config, const QueueStats& Stats);
    ...
    QueueResult
    LoadMessage(const std::string& QueueName, MessageId MessageId, MessagePtr& OutMessage);
