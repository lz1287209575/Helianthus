...
// 压缩和加密管理实现（拆分文件）
void MessageQueue::UpdateCompressionStats(const std::string& QueueName,
                                          ...
                                          double TimeMs)
{
    ...
    if (CompressedSize < OriginalSize)
    {
        ...
        {
            S.CompressionRatio = (S.CompressedBytes == 0)
                                     ? 0.0
                                     : (double)S.CompressedBytes / (double)S.OriginalBytes;
        }
...
QueueResult MessageQueue::ApplyCompression(MessagePtr Message, const std::string& QueueName)
{
    ...
    if (!Cfg.EnableAutoCompression)
        return QueueResult::SUCCESS;
    if (Message->Payload.Data.size() < Cfg.MinSize)
        return QueueResult::SUCCESS;
...
QueueResult MessageQueue::ApplyDecompression(MessagePtr Message, const std::string& QueueName)
{
    ...
    if (DidDecompress)
    {
        UpdateDecompressionStats(QueueName,
...
QueueResult MessageQueue::ApplyEncryption(MessagePtr Message, const std::string& QueueName)
{
    ...
    {
        H_LOG(
            MQ, Helianthus::Common::LogVerbosity::Verbose, "消息已加密，跳过: queue={}", QueueName);
        return QueueResult::SUCCESS;
    }
...
{
    if (!Message || !Message->Header.Properties.count("Encrypted"))
        return QueueResult::SUCCESS;
    auto Start = std::chrono::high_resolution_clock::now();
    ...
    if (R == QueueResult::SUCCESS)
    {
        UpdateDecryptionStats(QueueName,
                              std::chrono::duration<double, std::milli>(End - Start).count());
    }
    return R;
}
...
QueueResult MessageQueue::SetCompressionConfig(const std::string& QueueName,
...
QueueResult MessageQueue::GetCompressionConfig(const std::string& QueueName,
                                               CompressionConfig& OutConfig) const
{
...
QueueResult MessageQueue::SetEncryptionConfig(const std::string& QueueName,
                                              const EncryptionConfig& Config)
{
    ...
    H_LOG(MQ,
          ...
          QueueName,
          static_cast<int>(Config.Algorithm),
          Config.EnableAutoEncryption);
    return QueueResult::SUCCESS;
}

QueueResult MessageQueue::GetEncryptionConfig(const std::string& QueueName,
                                              EncryptionConfig& OutConfig) const
{
...
QueueResult MessageQueue::GetCompressionStats(const std::string& QueueName,
                                              CompressionStats& OutStats) const
{
...
QueueResult MessageQueue::GetEncryptionStats(const std::string& QueueName,
                                             EncryptionStats& OutStats) const
{
...
QueueResult MessageQueue::EncryptMessage(MessagePtr Message,
                                         ...
                                         const EncryptionConfig& Cfg)
{
    ...
    if (Algorithm == EncryptionAlgorithm::AES_128_CBC)
    {
        ...
        uint8_t Iv[16] = {0};
        if (!Cfg.Key.empty())
            memcpy(Key, Cfg.Key.data(), std::min<size_t>(Cfg.Key.size(), sizeof(Key)));
        if (!Cfg.IV.empty())
            memcpy(Iv, Cfg.IV.data(), std::min<size_t>(Cfg.IV.size(), sizeof(Iv)));
...
QueueResult MessageQueue::DecryptMessage(MessagePtr Message)
{
    ...
    EncryptionConfig Cfg{};
    {
        ...
        {
            auto It = std::find_if(EncryptionConfigs.begin(),
                                   EncryptionConfigs.end(),
                                   [](const auto& P) { return P.second.EnableAutoEncryption; });
            if (It != EncryptionConfigs.end())
                Cfg = It->second;
            else
                Cfg = EncryptionConfigs.begin()->second;
        }
    }
    ...
    {
        if (Message->Payload.Data.size() < 12 + 16)
            return QueueResult::INVALID_PARAMETER;
