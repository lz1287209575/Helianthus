...
JsonConfigProvider::JsonConfigProvider()
    : JsonData(std::make_unique<nlohmann::json>()), IsValidFlag(false), FileWatchingEnabled(false)
{
}
...
JsonConfigProvider::GetSection(const std::string& Prefix) const
{
    std::lock_guard<std::mutex> Lock(DataMutex);
...
void JsonConfigProvider::RegisterChangeCallback(const std::string& Key,
...
{
    return std::visit([](const auto& Val) { return nlohmann::json(Val); }, Value);
}

void JsonConfigProvider::CollectKeysRecursive(const nlohmann::json& JsonObj,
...
void JsonConfigProvider::NotifyConfigChange(const std::string& Key,
