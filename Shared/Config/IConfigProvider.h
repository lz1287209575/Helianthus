...
{
// 配置值类型
using ConfigValue = std::variant<std::string, int32_t, int64_t, uint32_t, uint64_t, double, bool>;
...
public:
    ...
    // 获取配置节（以前缀匹配）
    virtual std::unordered_map<std::string, ConfigValue>
    GetSection(const std::string& Prefix) const = 0;
