...
public:
    ...
    std::shared_ptr<IDatabase> CreateDatabase(DatabaseType Type,
                                              const std::string& ConnectionName = "default");
    std::shared_ptr<IDatabase>
    CreateDatabaseFromConfig(const std::string& ConnectionName = "default");

    // Utility methods
    static std::shared_ptr<DatabaseFactory> CreateDefault();
    static Common::ResultCode RegisterCustomFactory(
        DatabaseType Type,
        std::function<std::shared_ptr<IDatabase>(const std::string&)> FactoryFunction);

private:
    static std::map<DatabaseType, std::function<std::shared_ptr<IDatabase>(const std::string&)>>
        CustomFactories;
};
...
public:
    ...
    // Database management
    Common::ResultCode RegisterDatabase(const std::string& Name,
                                        DatabaseType Type,
                                        const std::string& ConnectionName = "default");
...
private:
    ...
    // Helper methods
    std::shared_ptr<IDatabase> CreateDatabaseInternal(DatabaseType Type,
