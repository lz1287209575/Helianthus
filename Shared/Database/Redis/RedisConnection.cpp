...
Common::ResultCode RedisConnection::Connect()
{
    ...
    if (!RedisContext || RedisContext->err)
    {
        ErrorCount++;
        std::string ErrorMsg =
            RedisContext ? RedisContext->errstr : "Failed to allocate redis context";
    ...
    if (!Config.Password.empty())
    {
        redisReply* Reply = static_cast<redisReply*>(
            redisCommand(RedisContext, "AUTH %s", Config.Password.c_str()));
    ...
    {
        redisReply* Reply =
            static_cast<redisReply*>(redisCommand(RedisContext, "SELECT %u", Config.Database));
        if (!Reply || Reply->type == REDIS_REPLY_ERROR)
        {
            ErrorCount++;
...
DatabaseResult RedisConnection::ExecuteQuery(const std::string& Query,
...
DatabaseResult RedisConnection::ExecuteStoredProcedure(const std::string& ProcedureName,
                                                       const ParameterMap& Parameters)
{
...
RedisResult
RedisConnection::Set(const std::string& Key, const std::string& Value, uint32_t ExpireSeconds)
{
    if (ExpireSeconds > 0)
    {
...
// Hash operations
RedisResult
RedisConnection::HashSet(const std::string& Key, const std::string& Field, const std::string& Value)
{
...
// List operations
RedisResult
RedisConnection::ListPush(const std::string& Key, const std::string& Value, bool PushLeft)
{
...
RedisResult
RedisConnection::SortedSetAdd(const std::string& Key, double Score, const std::string& Member)
{
...
RedisResult RedisConnection::SortedSetRange(const std::string& Key,
                                            int32_t Start,
                                            int32_t Stop,
                                            bool WithScores)
{
        ...
        if (redisGetReply(RedisContext, reinterpret_cast<void**>(&Reply)) == REDIS_OK)
        {
            ...
            if (CmdResult.IsSuccess())
            {
                Result.Values.insert(
                    Result.Values.end(), CmdResult.Values.begin(), CmdResult.Values.end());
            }
            freeReplyObject(Reply);
        }
    }
...
RedisTransaction::RedisTransaction(std::shared_ptr<RedisConnection> Connection)
    : Connection(Connection), IsActiveFlag(false)
{
}
...
DatabaseResult RedisTransaction::ExecuteQuery(const std::string& Query,
                                              const ParameterMap& Parameters)
{
    if (!IsActiveFlag)
    {
...
DatabaseResult RedisTransaction::ExecuteStoredProcedure(const std::string& ProcedureName,
                                                        const ParameterMap& Parameters)
{
    if (!IsActiveFlag)
    {
