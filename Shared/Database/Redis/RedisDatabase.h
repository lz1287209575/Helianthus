...
namespace Helianthus::Database::Redis
{
    ...
    public:
        ...
        DatabaseType GetDatabaseType() const override { return DatabaseType::REDIS; }
        ...
        Common::ResultCode SetCache(const std::string& Key, const std::string& Value, uint32_t ExpireSeconds = 0);
        std::string GetCache(const std::string& Key, bool& Found);
        Common::ResultCode DeleteCache(const std::string& Key);
        Common::ResultCode SetCacheHash(const std::string& Key, const std::map<std::string, std::string>& HashData, uint32_t ExpireSeconds = 0);
        std::map<std::string, std::string> GetCacheHash(const std::string& Key, bool& Found);

        // Session management
        Common::ResultCode SetSession(const std::string& SessionId, const std::string& Data, uint32_t ExpireSeconds = 3600);
        ...
        Common::ResultCode Subscribe(const std::string& Channel, std::function<void(const std::string&)> MessageHandler);
