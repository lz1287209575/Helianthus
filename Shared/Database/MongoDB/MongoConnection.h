...
public:
    ...
    MongoResult InsertMany(const std::string& Collection,
                           const std::vector<BsonDocument>& Documents);
    MongoResult FindOne(const std::string& Collection, const BsonDocument& Filter);
    MongoResult FindMany(const std::string& Collection,
                         const BsonDocument& Filter,
                         int32_t Limit = 0,
                         int32_t Skip = 0);
    MongoResult UpdateOne(const std::string& Collection,
                          const BsonDocument& Filter,
                          const BsonDocument& Update,
                          bool Upsert = false);
    MongoResult UpdateMany(const std::string& Collection,
                           const BsonDocument& Filter,
                           const BsonDocument& Update,
                           bool Upsert = false);
