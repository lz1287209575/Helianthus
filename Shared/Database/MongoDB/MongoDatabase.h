...
public:
    ...
    MongoResult InsertDocuments(const std::string& Collection,
                                const std::vector<BsonDocument>& Documents);
    MongoResult FindDocument(const std::string& Collection, const BsonDocument& Filter);
    MongoResult FindDocuments(const std::string& Collection,
                              const BsonDocument& Filter,
                              int32_t Limit = 0,
                              int32_t Skip = 0);
    MongoResult UpdateDocument(const std::string& Collection,
                               const BsonDocument& Filter,
                               const BsonDocument& Update,
                               bool Upsert = false);
    MongoResult UpdateDocuments(const std::string& Collection,
                                const BsonDocument& Filter,
                                const BsonDocument& Update,
                                bool Upsert = false);
    ...
    // 索引操作
    MongoResult CreateIndex(const std::string& Collection,
    ...
    // 配置和状态
    const MongoDbConfig& GetConfig() const
    {
        return Config;
    }
