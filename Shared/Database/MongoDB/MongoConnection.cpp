...
namespace Helianthus::Database::MongoDB
{
    ...
    Common::ResultCode MongoConnection::Connect()
    {
        ...
        try
        {
            ...
            if (Ping())
            {
                ...
                std::cout << "[MongoConnection] 连接成功: " << Config.Host << ":" << Config.Port << std::endl;
                return Common::ResultCode::SUCCESS;
            }
            else
            {
    ...
    DatabaseResult MongoConnection::ExecuteQuery(const std::string& Query, const ParameterMap& Parameters)
    {
        // MongoDB 不支持 SQL 查询，这里返回错误
        return DatabaseResult{
            .Code = Common::ResultCode::NOT_SUPPORTED,
            .ErrorMessage = "MongoDB does not support SQL queries"
        };
    }

    DatabaseResult MongoConnection::ExecuteStoredProcedure(const std::string& ProcedureName, const ParameterMap& Parameters)
    {
    ...
    MongoResult MongoConnection::InsertOne(const std::string& Collection, const BsonDocument& Document)
    {
        try
        {
            ...
            {
                return MongoResult{
                    .Code = Common::ResultCode::CONNECTION_ERROR,
            ...
            return MongoResult{
                .Code = Common::ResultCode::SUCCESS,
        ...
        catch (const std::exception& Ex)
        {
            ErrorCount++;
            return MongoResult{
                .Code = Common::ResultCode::QUERY_ERROR,
