...
namespace Helianthus::Database
{
    ...
    private:
        ...
        template<typename T>
        T GetConfigValue(const ConfigSection& Section, const std::string& Key, const T& DefaultValue) const;
    ...
    public:
        ...
        DatabaseConfigBuilder& MySqlCredentials(const std::string& Username, const std::string& Password);
        DatabaseConfigBuilder& MySqlSsl(bool EnableSsl);
        DatabaseConfigBuilder& MySqlTimeout(uint32_t ConnectionTimeout, uint32_t ReadTimeout, uint32_t WriteTimeout);
        ...
        DatabaseConfigBuilder& MongoCredentials(const std::string& Username, const std::string& Password);
