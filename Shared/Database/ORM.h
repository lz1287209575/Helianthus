...
namespace Helianthus::Database::ORM
{
    ...
    public:
        ...
        QueryBuilder& WhereBetween(const std::string& Column, const DatabaseValue& Start, const DatabaseValue& End);
    ...
    public:
        explicit Repository(std::shared_ptr<IDatabase> Database)
            : Database(Database)
        {
        }
        ...
        virtual std::vector<std::unique_ptr<TEntity>> FindWhere(const std::string& Condition, const ParameterMap& Parameters = {})
        {
            TEntity TempEntity;
            auto Query = QueryBuilder()
                .Select()
        ...
        virtual Common::ResultCode DeleteWhere(const std::string& Condition, const ParameterMap& Parameters = {})
        {
            TEntity TempEntity;
            auto Query = QueryBuilder()
                .DeleteFrom(TempEntity.GetTableName())
        ...
        // Pagination support
        virtual std::vector<std::unique_ptr<TEntity>> FindPage(uint32_t Page, uint32_t PageSize, const std::string& OrderBy = "")
        {
            TEntity TempEntity;
    ...
    {
    public:
        static Common::ResultCode CreateTable(std::shared_ptr<IDatabase> Database, const IEntity& Entity);
        static Common::ResultCode DropTable(std::shared_ptr<IDatabase> Database, const std::string& TableName);
        static Common::ResultCode CreateIndex(std::shared_ptr<IDatabase> Database, const std::string& TableName, 
                                            const std::vector<std::string>& Columns, const std::string& IndexName = "");
        ...
        static std::string GenerateCreateIndexSQL(const std::string& TableName, 
                                                 ...
                                                 const std::string& IndexName, DatabaseType DbType);
    };
