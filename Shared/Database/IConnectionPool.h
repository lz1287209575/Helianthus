...
namespace Helianthus::Database
{
    ...
    public:
        ...
        // Factory methods
        virtual std::shared_ptr<IConnectionPool> CreateMySqlPool(
            const MySqlConfig& DB_CONFIG, 
            const ConnectionPoolConfig& POOL_CONFIG) = 0;

        virtual std::shared_ptr<IConnectionPool> CreateMongoDbPool(
            const MongoDbConfig& DB_CONFIG, 
            const ConnectionPoolConfig& POOL_CONFIG) = 0;

        virtual std::shared_ptr<IConnectionPool> CreateRedisPool(
            const RedisConfig& DB_CONFIG, 
            const ConnectionPoolConfig& POOL_CONFIG) = 0;
    };
