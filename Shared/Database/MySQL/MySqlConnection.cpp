...
Common::ResultCode MySqlConnection::Connect()
{
    ...
    // Attempt connection
    MYSQL* Result = mysql_real_connect(MySqlConnectionHandle,
                                       Config.Host.c_str(),
...
DatabaseResult MySqlConnection::ExecuteQuery(const std::string& Query,
...
DatabaseResult MySqlConnection::ExecuteStoredProcedure(const std::string& ProcedureName,
                                                       const ParameterMap& Parameters)
{
...
std::string MySqlConnection::PrepareQuery(const std::string& Query,
...
{
    std::stringstream Ss;
    Ss << "mysql://" << Config.Username << "@" << Config.Host << ":" << Config.Port << "/"
       << Config.Database;
    return Ss.str();
}

// MySqlTransaction implementation
MySqlTransaction::MySqlTransaction(std::shared_ptr<MySqlConnection> Connection,
                                   IsolationLevel Level)
    : Connection(Connection), TransactionIsolationLevel(Level), IsActiveFlag(false)
{
}
...
DatabaseResult MySqlTransaction::ExecuteQuery(const std::string& Query,
                                              const ParameterMap& Parameters)
{
    if (!IsActiveFlag)
    {
...
DatabaseResult MySqlTransaction::ExecuteStoredProcedure(const std::string& ProcedureName,
                                                        const ParameterMap& Parameters)
{
    if (!IsActiveFlag)
    {
