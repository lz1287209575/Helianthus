...
// 预定义的直方图桶（毫秒）
const std::vector<double> LatencyHistogram::HistogramBuckets = {0.001,
                                                                0.005,
                                                                0.01,
                                                                0.025,
                                                                0.05,
...
void TransactionPerformanceStats::AddCommitSample(uint64_t DurationNs)
{
    ...
    while (DurationNs < CurrentMin &&
           !MinCommitTimeNs.compare_exchange_weak(CurrentMin, DurationNs))
    {
        // 重试直到成功更新最小值
    }
    ...
    while (DurationNs > CurrentMax &&
           !MaxCommitTimeNs.compare_exchange_weak(CurrentMax, DurationNs))
    {
        // 重试直到成功更新最大值
    }
...
void TransactionPerformanceStats::AddRollbackSample(uint64_t DurationNs)
{
    ...
    while (DurationNs < CurrentMin &&
           !MinRollbackTimeNs.compare_exchange_weak(CurrentMin, DurationNs))
    {
        // 重试直到成功更新最小值
    }
    ...
    while (DurationNs > CurrentMax &&
           !MaxRollbackTimeNs.compare_exchange_weak(CurrentMax, DurationNs))
    {
        // 重试直到成功更新最大值
    }
...
void TransactionPerformanceStats::UpdateTransactionCount(bool Committed,
                                                         bool RolledBack,
                                                         bool Timeout,
                                                         bool Failed)
{
...
TransactionPerformanceStats::GetRollbackTimeHistogram() const
{
    std::lock_guard<std::mutex> Lock(HistogramMutex);
    if (!RollbackTimeHistogramPtr)
        return {};
    ...
    while (Running.load())
    {
        ...
        else if (Path == "/metrics")
        {
            ...
            else
            {
                ...
                if (Body.empty())
                    Body =
                        "# HELP helianthus_up 1 if exporter is up\n# TYPE helianthus_up "
                        "gauge\nhelianthus_up 1\n";
...
void EnhancedPrometheusExporter::UpdateBatchPerformance(const std::string& QueueName,
                                                        uint64_t DurationNs,
...
void EnhancedPrometheusExporter::UpdateTransactionPerformance(bool Committed,
                                                              bool RolledBack,
                                                              bool Timeout,
                                                              bool Failed,
                                                              uint64_t CommitTimeNs,
                                                              uint64_t RollbackTimeNs)
{
...
const BatchPerformanceStats&
EnhancedPrometheusExporter::GetBatchStats(const std::string& QueueName) const
{
    std::lock_guard<std::mutex> Lock(BatchStatsMutex);
...
std::string EnhancedPrometheusExporter::FormatMetric(
...
std::string
EnhancedPrometheusExporter::FormatHistogram(const std::string& Name,
...
std::string EnhancedPrometheusExporter::FormatCounter(
    const std::string& Name, uint64_t Value, const std::map<std::string, std::string>& Labels)
{
    std::ostringstream Oss;
    Oss << Name;
...
std::string EnhancedPrometheusExporter::ExportBatchMetrics() const
{
    ...
    Oss << "# HELP helianthus_batch_duration_p50_ms 50th percentile of batch processing duration "
           "in milliseconds\n";
    Oss << "# TYPE helianthus_batch_duration_p50_ms gauge\n";
    Oss << "# HELP helianthus_batch_duration_p95_ms 95th percentile of batch processing duration "
           "in milliseconds\n";
    Oss << "# TYPE helianthus_batch_duration_p95_ms gauge\n";
    Oss << "# HELP helianthus_batch_duration_p99_ms 99th percentile of batch processing duration "
           "in milliseconds\n";
    Oss << "# TYPE helianthus_batch_duration_p99_ms gauge\n";
    Oss << "# HELP helianthus_batch_duration_avg_ms Average batch processing duration in "
           "milliseconds\n";
    ...
    for (const auto& Pair : BatchStatsByQueue)
    {
        ...
        Oss << FormatMetric(
            "helianthus_batch_duration_avg_ms", Stats.GetAverageDurationMs(), Labels);
    }
...
std::string EnhancedPrometheusExporter::ExportZeroCopyMetrics() const
{
    ...
    // 添加分位数指标的 HELP/TYPE 注释
    Oss << "# HELP helianthus_zero_copy_duration_p50_ms 50th percentile of zero-copy operation "
           "duration in milliseconds\n";
    Oss << "# TYPE helianthus_zero_copy_duration_p50_ms gauge\n";
    Oss << "# HELP helianthus_zero_copy_duration_p95_ms 95th percentile of zero-copy operation "
           "duration in milliseconds\n";
    Oss << "# TYPE helianthus_zero_copy_duration_p95_ms gauge\n";
    Oss << "# HELP helianthus_zero_copy_duration_p99_ms 99th percentile of zero-copy operation "
           "duration in milliseconds\n";
    Oss << "# TYPE helianthus_zero_copy_duration_p99_ms gauge\n";
    Oss << "# HELP helianthus_zero_copy_duration_avg_ms Average zero-copy operation duration in "
           "milliseconds\n";
    ...
    // 导出计数器
    Oss << FormatCounter(
        "helianthus_zero_copy_operations_total", ZeroCopyStats.TotalOperations.load(), {});

    // 导出分位数
    Oss << FormatMetric(
        "helianthus_zero_copy_duration_p50_ms", ZeroCopyStats.GetP50DurationMs(), {});
    Oss << FormatMetric(
        "helianthus_zero_copy_duration_p95_ms", ZeroCopyStats.GetP95DurationMs(), {});
    Oss << FormatMetric(
        "helianthus_zero_copy_duration_p99_ms", ZeroCopyStats.GetP99DurationMs(), {});
    Oss << FormatMetric(
        "helianthus_zero_copy_duration_avg_ms", ZeroCopyStats.GetAverageDurationMs(), {});
...
std::string EnhancedPrometheusExporter::ExportTransactionMetrics() const
{
    ...
    // 添加 HELP 和 TYPE 注释
    Oss << "# HELP helianthus_transaction_commit_duration_ms Transaction commit duration in "
           "milliseconds\n";
    Oss << "# TYPE helianthus_transaction_commit_duration_ms histogram\n";
    Oss << "# HELP helianthus_transaction_rollback_duration_ms Transaction rollback duration in "
           "milliseconds\n";
    ...
    Oss << "# HELP helianthus_transaction_rolled_back_total Total number of rolled back "
           "transactions\n";
    ...
    // 添加分位数指标的 HELP/TYPE 注释
    Oss << "# HELP helianthus_transaction_commit_duration_p50_ms 50th percentile of transaction "
           "commit duration in milliseconds\n";
    Oss << "# TYPE helianthus_transaction_commit_duration_p50_ms gauge\n";
    Oss << "# HELP helianthus_transaction_commit_duration_p95_ms 95th percentile of transaction "
           "commit duration in milliseconds\n";
    Oss << "# TYPE helianthus_transaction_commit_duration_p95_ms gauge\n";
    Oss << "# HELP helianthus_transaction_commit_duration_p99_ms 99th percentile of transaction "
           "commit duration in milliseconds\n";
    Oss << "# TYPE helianthus_transaction_commit_duration_p99_ms gauge\n";
    Oss << "# HELP helianthus_transaction_commit_duration_avg_ms Average transaction commit "
           "duration in milliseconds\n";
    Oss << "# TYPE helianthus_transaction_commit_duration_avg_ms gauge\n";

    Oss << "# HELP helianthus_transaction_rollback_duration_p50_ms 50th percentile of transaction "
           "rollback duration in milliseconds\n";
    Oss << "# TYPE helianthus_transaction_rollback_duration_p50_ms gauge\n";
    Oss << "# HELP helianthus_transaction_rollback_duration_p95_ms 95th percentile of transaction "
           "rollback duration in milliseconds\n";
    Oss << "# TYPE helianthus_transaction_rollback_duration_p95_ms gauge\n";
    Oss << "# HELP helianthus_transaction_rollback_duration_p99_ms 99th percentile of transaction "
           "rollback duration in milliseconds\n";
    Oss << "# TYPE helianthus_transaction_rollback_duration_p99_ms gauge\n";
    Oss << "# HELP helianthus_transaction_rollback_duration_avg_ms Average transaction rollback "
           "duration in milliseconds\n";
    ...
    {
        Oss << FormatHistogram(
            "helianthus_transaction_rollback_duration_ms", RollbackHistogram, {});
    }

    // 导出计数器
    Oss << FormatCounter(
        "helianthus_transaction_total", TransactionStats.TotalTransactions.load(), {});
    Oss << FormatCounter("helianthus_transaction_committed_total",
                         TransactionStats.CommittedTransactions.load(),
                         {});
    Oss << FormatCounter("helianthus_transaction_rolled_back_total",
                         TransactionStats.RolledBackTransactions.load(),
                         {});
    Oss << FormatCounter(
        "helianthus_transaction_timeout_total", TransactionStats.TimeoutTransactions.load(), {});
    Oss << FormatCounter(
        "helianthus_transaction_failed_total", TransactionStats.FailedTransactions.load(), {});

    // 导出分位数
    Oss << FormatMetric(
        "helianthus_transaction_commit_duration_p50_ms", TransactionStats.GetP50CommitTimeMs(), {});
    Oss << FormatMetric(
        "helianthus_transaction_commit_duration_p95_ms", TransactionStats.GetP95CommitTimeMs(), {});
    Oss << FormatMetric(
        "helianthus_transaction_commit_duration_p99_ms", TransactionStats.GetP99CommitTimeMs(), {});
    Oss << FormatMetric("helianthus_transaction_commit_duration_avg_ms",
                        TransactionStats.GetAverageCommitTimeMs(),
                        {});

    Oss << FormatMetric("helianthus_transaction_rollback_duration_p50_ms",
                        TransactionStats.GetP50RollbackTimeMs(),
                        {});
    Oss << FormatMetric("helianthus_transaction_rollback_duration_p95_ms",
                        TransactionStats.GetP95RollbackTimeMs(),
                        {});
    Oss << FormatMetric("helianthus_transaction_rollback_duration_p99_ms",
                        TransactionStats.GetP99RollbackTimeMs(),
                        {});
    Oss << FormatMetric("helianthus_transaction_rollback_duration_avg_ms",
                        TransactionStats.GetAverageRollbackTimeMs(),
                        {});

    // 导出比率
    Oss << FormatMetric(
        "helianthus_transaction_success_rate", TransactionStats.GetSuccessRate(), {});
    Oss << FormatMetric(
        "helianthus_transaction_rollback_rate", TransactionStats.GetRollbackRate(), {});
    Oss << FormatMetric(
        "helianthus_transaction_timeout_rate", TransactionStats.GetTimeoutRate(), {});
    Oss << FormatMetric(
        "helianthus_transaction_failure_rate", TransactionStats.GetFailureRate(), {});
