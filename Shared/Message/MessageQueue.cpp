...
std::vector<MessagePtr> MessageQueue::FindMessages(std::function<bool(const MessagePtr&)> Predicate,
...
MessagePtr MessageQueue::FindFirstMessage(MessageType MsgType) const
{
    auto Messages = FindMessages(
        [MsgType](const MessagePtr& Message) { return Message->GetMessageType() == MsgType; }, 1);
...
MessageResult MessageQueue::WaitForMessage(MessageType MsgType, uint32_t TimeoutMs)
{
    ...
    auto WaitResult = QueueCondition.wait_for(
        Lock,
        std::chrono::milliseconds(TimeoutMs),
        [this, MsgType] { return FindFirstMessage(MsgType) != nullptr || ShuttingDownFlag; });
