...
public:
    ...
    std::vector<MessagePtr> FindMessages(std::function<bool(const MessagePtr&)> Predicate,
                                         uint32_t MaxCount = 10) const override;
...
private:
    ...
    using PriorityQueue =
        std::priority_queue<MessagePtr, std::vector<MessagePtr>, MessagePriorityComparator>;
