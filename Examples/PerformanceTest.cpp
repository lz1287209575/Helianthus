...
int main()
{
    ...
    auto CreateZeroCopyResult = Queue->CreateZeroCopyBuffer(TestData.data(), TestData.size(), ZeroCopyBuffer);
    ...
    if (GetStatsResult == QueueResult::SUCCESS)
    {
        ...
        std::cout << "  内存池命中率: " << (PerformanceStats.MemoryPoolHitRate * 100) << "%" << std::endl;
        std::cout << "  零拷贝操作次数: " << PerformanceStats.ZeroCopyOperations << std::endl;
        std::cout << "  批处理操作次数: " << PerformanceStats.BatchOperations << std::endl;
        std::cout << "  平均分配时间: " << PerformanceStats.AverageAllocationTimeMs << "ms" << std::endl;
        std::cout << "  平均释放时间: " << PerformanceStats.AverageDeallocationTimeMs << "ms" << std::endl;
        std::cout << "  平均零拷贝时间: " << PerformanceStats.AverageZeroCopyTimeMs << "ms" << std::endl;
        std::cout << "  平均批处理时间: " << PerformanceStats.AverageBatchTimeMs << "ms" << std::endl;
    }
    ...
    if (GetMemoryPoolResult == QueueResult::SUCCESS)
    {
        ...
        std::cout << "  初始大小: " << RetrievedMemoryPoolConfig.InitialSize << " bytes" << std::endl;
        ...
        std::cout << "  启用预分配: " << (RetrievedMemoryPoolConfig.EnablePreallocation ? "是" : "否") << std::endl;
        std::cout << "  预分配块数: " << RetrievedMemoryPoolConfig.PreallocationBlocks << std::endl;
        std::cout << "  启用压缩: " << (RetrievedMemoryPoolConfig.EnableCompaction ? "是" : "否") << std::endl;
        std::cout << "  压缩阈值: " << RetrievedMemoryPoolConfig.CompactionThreshold << "%" << std::endl;
    }
    ...
    if (GetBufferResult == QueueResult::SUCCESS)
    {
        ...
        std::cout << "  初始容量: " << RetrievedBufferConfig.InitialCapacity << " bytes" << std::endl;
        std::cout << "  最大容量: " << RetrievedBufferConfig.MaxCapacity << " bytes" << std::endl;
        std::cout << "  增长因子: " << RetrievedBufferConfig.GrowthFactor << std::endl;
        std::cout << "  启用零拷贝: " << (RetrievedBufferConfig.EnableZeroCopy ? "是" : "否") << std::endl;
        std::cout << "  启用压缩: " << (RetrievedBufferConfig.EnableCompression ? "是" : "否") << std::endl;
        std::cout << "  压缩阈值: " << RetrievedBufferConfig.CompressionThreshold << " bytes" << std::endl;
        std::cout << "  启用批处理: " << (RetrievedBufferConfig.EnableBatching ? "是" : "否") << std::endl;
