...
private:
    ...
    void TestPriorityQueue()
    {
        ...
        for (const auto& [priority, content] : testMessages)
        {
            ...
            std::cout << "✓ 创建 " << static_cast<int>(priority) << " 优先级消息: " 
    ...
    void TestPublishSubscribe()
    {
        ...
        // 创建游戏事件消息
        auto gameEvent = std::make_shared<Message>(MessageType::PLAYER_EVENT, 
            R"({"event": "player_join", "player_id": 123, "level": 50})");
        
        std::cout << "✓ 创建游戏事件消息: " << gameEvent->Payload.AsString() << std::endl;
        
        // 创建系统通知消息
        auto systemNotification = std::make_shared<Message>(MessageType::SYSTEM_NOTIFICATION,
    ...
    void TestBatchOperations()
    {
        ...
        {
            std::cout << "  " << (i + 1) << ". " << batchMessages[i]->Payload.AsString() << std::endl;
        }
    }
    ...
    void TestQueueConfig()
    {
        ...
        std::cout << "  启用批量处理: " << (standardConfig.EnableBatching ? "是" : "否") << std::endl;
    ...
    void TestTopicConfig()
    {
        ...
        std::cout << "  启用分区: " << (gameTopicConfig.EnablePartitioning ? "是" : "否") << std::endl;
    ...
    void TestProducerConfig()
    {
        ...
        std::cout << "  等待确认: " << (producerConfig.WaitForAcknowledge ? "是" : "否") << std::endl;
