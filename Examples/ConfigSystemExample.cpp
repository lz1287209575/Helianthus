...
{
    std::cout << "Server port changed from " << OldValue.AsInt() 
              << " to " << NewValue.AsInt() << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        std::cout << "  主机: " << (ServerHost.IsValid() ? ServerHost.AsString() : "localhost") << std::endl;
        std::cout << "  端口: " << (ServerPort.IsValid() ? ServerPort.AsInt() : 8080) << std::endl;
        std::cout << "  最大连接数: " << (MaxConnections.IsValid() ? MaxConnections.AsInt() : 100) << std::endl;
        std::cout << "  启用SSL: " << (EnableSSL.IsValid() ? (EnableSSL.AsBool() ? "是" : "否") : "否") << std::endl;
        ...
        std::cout << "  主机: " << (DbHost.IsValid() ? DbHost.AsString() : "localhost") << std::endl;
