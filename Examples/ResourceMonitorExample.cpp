...
void OnResourceStatsUpdate(const ResourceUsageStats& Stats)
{
    ...
    // CPU 信息
    std::cout << "\n--- CPU 信息 ---" << std::endl;
    std::cout << "CPU 使用率: " << std::fixed << std::setprecision(2) << Stats.CpuUsagePercent
              << "%" << std::endl;
    ...
    std::cout << "内存使用率: " << std::fixed << std::setprecision(2) << Stats.MemoryUsagePercent
              << "%" << std::endl;

    if (Stats.SwapTotalBytes > 0)
    {
        std::cout << "交换分区: " << FormatBytes(Stats.SwapTotalBytes)
                  << " (已使用: " << FormatBytes(Stats.SwapUsedBytes) << ", " << std::fixed
                  << std::setprecision(2) << Stats.SwapUsagePercent << "%)" << std::endl;
    }
    ...
    if (!Stats.DiskStatsList.empty())
    {
        ...
        for (const auto& Disk : Stats.DiskStatsList)
        {
            ...
            std::cout << "  使用率: " << std::fixed << std::setprecision(2) << Disk.UsagePercent
                      << "%" << std::endl;

            if (Disk.ReadBytesPerSec > 0 || Disk.WriteBytesPerSec > 0)
            {
                std::cout << "  读取速率: " << FormatBytes(Disk.ReadBytesPerSec) << "/s"
                          << std::endl;
                std::cout << "  写入速率: " << FormatBytes(Disk.WriteBytesPerSec) << "/s"
                          << std::endl;
            }
        }
    }
    ...
    if (!Stats.NetworkStatsList.empty())
    {
        ...
        for (const auto& Net : Stats.NetworkStatsList)
        {
            ...
            std::cout << "  接收包数: " << Net.PacketsReceived << " (" << Net.PacketsReceivedPerSec
                      << "/s)" << std::endl;
            std::cout << "  发送包数: " << Net.PacketsSent << " (" << Net.PacketsSentPerSec << "/s)"
                      << std::endl;
...
int main()
{
    ...
    if (!HistoryStats.empty())
    {
        ...
        std::cout << "最新记录时间: "
                  << std::chrono::duration_cast<std::chrono::seconds>(
                         LatestStats.Timestamp.time_since_epoch())
                         .count()
                  << "s" << std::endl;
        ...
        if (ValidCount > 0)
        {
            ...
            std::cout << "平均 CPU 使用率: " << std::fixed << std::setprecision(2) << AvgCpuUsage
                      << "%" << std::endl;
            std::cout << "平均内存使用率: " << std::fixed << std::setprecision(2) << AvgMemoryUsage
                      << "%" << std::endl;
        }
    }
