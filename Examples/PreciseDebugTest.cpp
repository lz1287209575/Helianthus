...
int main()
{
    ...
    if (initResult != QueueResult::SUCCESS) {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Error, "步骤4失败：初始化失败 code={}", static_cast<int>(initResult));
        return 1;
    }
    
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "步骤4完成：FileBasedPersistence初始化成功");
    
    // 步骤5：测试基本操作
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "步骤5：测试基本操作");
    try {
        auto queues = filePersistence->ListPersistedQueues();
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "步骤5完成：ListPersistedQueues成功，队列数量: {}", queues.size());
    } catch (const std::exception& e) {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Error, "步骤5失败：ListPersistedQueues异常: {}", e.what());
        return 1;
    }
    ...
    try {
        ...
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "步骤6完成：FileBasedPersistence关闭成功");
    } catch (const std::exception& e) {
    ...
    try {
        ...
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "步骤7完成：FileBasedPersistence析构成功");
    } catch (const std::exception& e) {
