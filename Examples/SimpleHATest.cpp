...
int main()
{
    ...
    Queue->SetLeaderChangeHandler(
        ...
        {
            std::cout << "Leader变更: shard=" << Shard << ", old=" << OldLeader
                      << ", new=" << NewLeader << std::endl;
        });

    Queue->SetFailoverHandler(
        [](ShardId Shard, const std::string& FailedLeader, const std::string& TakeoverNode)
        {
            std::cout << "Failover发生: shard=" << Shard << ", failed_leader=" << FailedLeader
                      << ", takeover=" << TakeoverNode << std::endl;
        });
    ...
    for (int i = 1; i <= 3; ++i)
    {
        ...
        if (SendResult == QueueResult::SUCCESS)
        {
            std::cout << "发送消息成功: id=" << Message->Header.Id
                      << ", partition_key=" << Message->Header.Properties["partition_key"]
                      << std::endl;
        }
        else
        {
    ...
    if (StatusResult == QueueResult::SUCCESS)
    {
        for (const auto& Shard : ShardStatuses)
        {
            ...
            std::cout << "分片状态: shard=" << Shard.Id << ", leader=" << LeaderNode
