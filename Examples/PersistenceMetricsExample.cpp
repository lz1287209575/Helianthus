...
int main()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        if (Result == QueueResult::SUCCESS)
        {
            std::cout << "接收消息 ID: " << static_cast<uint64_t>(ReceivedMessage->Header.Id)
                      << std::endl;
    ...
    std::cout << "  平均写入时间: " << PersistenceStats.GetAverageWriteTimeMs() << " ms"
              << std::endl;
    ...
    std::cout << "  平均读取时间: " << PersistenceStats.GetAverageReadTimeMs() << " ms"
              << std::endl;
