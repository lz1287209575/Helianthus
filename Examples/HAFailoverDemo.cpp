...
int main()
{
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "集群配置设置完成: 2个分片，每个分片2个副本");
    ...
    for (int i = 1; i <= 5; ++i)
    {
        ...
        {
            H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "发送消息成功: id={}, partition_key={}", 
                  Message->Header.Id, Message->Header.Properties["partition_key"]);
        }
        else
        {
            H_LOG(MQ, Helianthus::Common::LogVerbosity::Error, "发送消息失败: id={}, error={}", 
                  Message->Header.Id, static_cast<int>(SendResult));
        }
    ...
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, 
              "队列指标: queue={}, pending={}, total={}, processed={}, dlq={}, retried={}, enq_rate={:.2f}/s, deq_rate={:.2f}/s, p50={:.2f}ms, p95={:.2f}ms",
              Metrics.QueueName, Metrics.PendingMessages, Metrics.TotalMessages, 
              Metrics.ProcessedMessages, Metrics.DeadLetterMessages, Metrics.RetriedMessages,
              Metrics.EnqueueRate, Metrics.DequeueRate, Metrics.P50LatencyMs, Metrics.P95LatencyMs);
    }
