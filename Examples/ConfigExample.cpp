...
int main()
{
    ...
    std::cout << "默认配置加载完成，配置项数量: " << ConfigManager->GetConfigItemCount() << std::endl;
    ...
    std::cout << "应用环境: " << ConfigManager->GetString("app.environment", "Unknown") << std::endl;
    std::cout << "调试模式: " << (ConfigManager->GetBool("app.debug", false) ? "开启" : "关闭") << std::endl;
    ...
    // 消息队列配置
    std::cout << "\n消息队列配置:" << std::endl;
    std::cout << "  最大消息数: " << ConfigManager->GetInt("messagequeue.max_size", 10000) << std::endl;
    std::cout << "  最大字节数: " << ConfigManager->GetInt("messagequeue.max_size_bytes", 100 * 1024 * 1024) << std::endl;
    std::cout << "  最大消费者: " << ConfigManager->GetInt("messagequeue.max_consumers", 100) << std::endl;
    std::cout << "  最大生产者: " << ConfigManager->GetInt("messagequeue.max_producers", 100) << std::endl;
    std::cout << "  消息TTL: " << ConfigManager->GetInt("messagequeue.message_ttl_ms", 300000) << "ms" << std::endl;
    std::cout << "  启用死信队列: " << (ConfigManager->GetBool("messagequeue.enable_dead_letter", true) ? "是" : "否") << std::endl;
    std::cout << "  启用批处理: " << (ConfigManager->GetBool("messagequeue.enable_batching", true) ? "是" : "否") << std::endl;
    std::cout << "  批处理大小: " << ConfigManager->GetInt("messagequeue.batch_size", 100) << std::endl;

    // 网络配置
    std::cout << "\n网络配置:" << std::endl;
    std::cout << "  最大连接数: " << ConfigManager->GetInt("network.max_connections", 1000) << std::endl;
    std::cout << "  连接超时: " << ConfigManager->GetInt("network.connection_timeout_ms", 30000) << "ms" << std::endl;
    std::cout << "  读取超时: " << ConfigManager->GetInt("network.read_timeout_ms", 60000) << "ms" << std::endl;
    std::cout << "  写入超时: " << ConfigManager->GetInt("network.write_timeout_ms", 60000) << "ms" << std::endl;
    std::cout << "  启用压缩: " << (ConfigManager->GetBool("network.enable_compression", true) ? "是" : "否") << std::endl;
    std::cout << "  启用加密: " << (ConfigManager->GetBool("network.enable_encryption", false) ? "是" : "否") << std::endl;
    std::cout << "  压缩算法: " << ConfigManager->GetString("network.compression_algorithm", "gzip") << std::endl;
    std::cout << "  加密算法: " << ConfigManager->GetString("network.encryption_algorithm", "aes-256-gcm") << std::endl;
    ...
    std::cout << "  日志输出: " << ConfigManager->GetString("logging.output", "console") << std::endl;
    std::cout << "  日志文件: " << ConfigManager->GetString("logging.file_path", "logs/helianthus.log") << std::endl;
    std::cout << "  启用轮转: " << (ConfigManager->GetBool("logging.enable_rotation", true) ? "是" : "否") << std::endl;
    std::cout << "  最大文件大小: " << ConfigManager->GetInt("logging.max_file_size_mb", 100) << "MB" << std::endl;
    std::cout << "  最大文件数: " << ConfigManager->GetInt("logging.max_files", 10) << std::endl;

    // 监控配置
    std::cout << "\n监控配置:" << std::endl;
    std::cout << "  启用指标: " << (ConfigManager->GetBool("monitoring.enable_metrics", true) ? "是" : "否") << std::endl;
    std::cout << "  指标端口: " << ConfigManager->GetInt("monitoring.metrics_port", 9090) << std::endl;
    std::cout << "  指标路径: " << ConfigManager->GetString("monitoring.metrics_path", "/metrics") << std::endl;
    std::cout << "  启用健康检查: " << (ConfigManager->GetBool("monitoring.enable_health_check", true) ? "是" : "否") << std::endl;
    std::cout << "  健康检查间隔: " << ConfigManager->GetInt("monitoring.health_check_interval_ms", 30000) << "ms" << std::endl;
    ...
    std::cout << "新的调试模式: " << (ConfigManager->GetBool("app.debug") ? "开启" : "关闭") << std::endl;
    ...
    ConfigChangeCallback ChangeCallback = [](const std::string& Key, const ConfigValue& OldValue, const ConfigValue& NewValue) {
    ...
    std::cout << "  启用SSL: " << (ConfigManager->GetBool("security.enable_ssl") ? "是" : "否") << std::endl;
    ...
    std::cout << "  线程池大小: " << ConfigManager->GetInt("performance.thread_pool_size") << std::endl;
    std::cout << "  最大连接数: " << ConfigManager->GetInt("performance.max_connections") << std::endl;
    std::cout << "  启用缓存: " << (ConfigManager->GetBool("performance.enable_caching") ? "是" : "否") << std::endl;
    ...
    std::string FilePath = TempDir ? std::string(TempDir) + "/helianthus_modified.conf" : "/tmp/helianthus_modified.conf";
    ...
    std::cout << "配置验证状态: " << (ConfigManager->ValidateConfig() ? "通过" : "失败") << std::endl;
