...
int main()
{
    ...
    {
        std::cout << "查询压缩配置成功: algorithm="
                  << static_cast<int>(RetrievedCompConfig.Algorithm)
                  << ", level=" << RetrievedCompConfig.Level
                  << ", min_size=" << RetrievedCompConfig.MinSize << std::endl;
    }
    ...
    {
        std::cout << "查询加密配置成功: algorithm="
                  << static_cast<int>(RetrievedEncConfig.Algorithm)
    ...
    {
        std::cout << "消息压缩成功，压缩后大小: " << Message->Payload.Data.size() << " 字节"
                  << std::endl;
    }
    else
    {
    ...
    // 加密消息
    auto EncryptResult =
        Queue->EncryptMessage(Message, EncryptionAlgorithm::AES_256_GCM, EncConfig);
    if (EncryptResult == QueueResult::SUCCESS)
    {
        std::cout << "消息加密成功，加密后大小: " << Message->Payload.Data.size() << " 字节"
                  << std::endl;
    }
    else
    {
    ...
    for (int tries = 0; tries < 4 && !Verified; ++tries)
    {
        ...
        {
            // 详细检查消息属性
            std::cout << "接收消息 ID=" << RecvMsg->Header.Id
                      << " size=" << RecvMsg->Payload.Data.size() << std::endl;
    ...
    std::cout << "批量发送: N=" << Sent << ", 总耗时(ms)=" << Ms << ", 吞吐(msg/s)=" << Throughput
