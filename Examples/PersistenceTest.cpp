...
int main()
{
    ...
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Error, "持久化管理器初始化失败 code={}", static_cast<int>(initResult));
        return 1;
    }
    ...
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Error, "队列配置保存失败 code={}", static_cast<int>(saveResult));
    }
    ...
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "队列配置加载成功: name={}, maxSize={}", 
              loadedConfig.Name, loadedConfig.MaxSize);
    }
    else
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Error, "队列配置加载失败 code={}", static_cast<int>(loadResult));
    }
