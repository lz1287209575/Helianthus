...
int main()
{
    try
    {
        ...
        NameProp.Setter = [](void* Obj, void* Value)
        { static_cast<HObject*>(Obj)->Name = *static_cast<std::string*>(Value); };
        ...
        IDProp.Setter = [](void* Obj, void* Value)
        { static_cast<HObject*>(Obj)->ID = *static_cast<int32_t*>(Value); };
        ...
        { static_cast<HPlayer*>(Obj)->Health = *static_cast<int32_t*>(Value); };
        ...
        SpeedProp.Setter = [](void* Obj, void* Value)
        { static_cast<HPlayer*>(Obj)->Speed = *static_cast<float*>(Value); };
        ...
        IsAliveProp.Getter = [](void* Obj) -> void*
        { return &static_cast<HPlayer*>(Obj)->IsAlive; };
        IsAliveProp.Setter = [](void* Obj, void* Value)
        { static_cast<HPlayer*>(Obj)->IsAlive = *static_cast<bool*>(Value); };
        ...
        for (const auto& Name : HClassNames)
        {
            ...
            if (Info)
            {
                ...
                {
                    const HPropertyInfo* PropInfo =
                        HelianthusReflectionSystem.GetHPropertyInfo(Name, PropName);
                    if (PropInfo)
                    {
                ...
                {
                    const HFunctionInfo* FuncInfo =
                        HelianthusReflectionSystem.GetHFunctionInfo(Name, FuncName);
                    if (FuncInfo)
                    {
        ...
        if (PlayerObj)
        {
            ...
            Args.clear();
            void* StatusResult =
                HelianthusReflectionSystem.CallHFunction(PlayerObj, "GetStatus", Args);
            if (StatusResult)
            {
        ...
        // 保存脚本绑定
        bool SaveResult =
            HelianthusReflectionSystem.SaveScriptBindings("helianthus_bindings.lua", "lua");
        if (SaveResult)
        {
