...
// 使用简化的运行时反射宏定义类
HELIANTHUS_CLASS(HPlayer, HObject){public : HPlayer() : Health(100),
                                   Speed(5.0f),
                                   Name("DefaultPlayer"){}

                                   void TakeDamage(int32_t damage){Health -= damage;
if (Health < 0)
    Health = 0;
...
void Heal(int32_t amount)
{
    ...
    if (Health > 100)
        Health = 100;
    std::cout << "Player " << Name << " healed " << amount << " HP. Health: " << Health
              << std::endl;
}

std::string GetStatus() const
{
    return "Player: " + Name + ", Health: " + std::to_string(Health) +
...
{
    if (PropertyName == "Health")
        Health = *static_cast<int32_t*>(Value);
    if (PropertyName == "Speed")
...
// 第二个类
HELIANTHUS_CLASS(HWeapon, HObject){public : HWeapon() : WeaponName("DefaultWeapon"),
                                   Damage(10),
                                   Range(100.0f){}

                                   void Upgrade(){Damage += 5;
Range += 10.0f;
std::cout << "Weapon " << WeaponName << " upgraded! Damage: " << Damage << ", Range: " << Range
          << std::endl;
}

std::string GetWeaponInfo() const
{
    return "Weapon: " + WeaponName + ", Damage: " + std::to_string(Damage) +
...
{
    if (PropertyName == "WeaponName")
        WeaponName = *static_cast<std::string*>(Value);
    if (PropertyName == "Damage")
        Damage = *static_cast<int32_t*>(Value);
    if (PropertyName == "Range")
        Range = *static_cast<float*>(Value);
}
...
// 测试枚举
HELIANTHUS_ENUM(HWeaponType){SWORD = 0, BOW = 1, STAFF = 2, DAGGER = 3};
...
int main()
{
    ...
    WeaponTypeEnum.EnumValues = {{"SWORD", static_cast<int32_t>(HWeaponType::SWORD)},
