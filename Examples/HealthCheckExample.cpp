...
void OnHealthCheck(HealthCheckType Type, const HealthCheckStatus& Status)
{
    std::cout << "🔍 健康检查: " << static_cast<int>(Type) << " = "
              << static_cast<int>(Status.Result) << " (" << Status.Message << ")" << std::endl;
}
...
void OnOverallHealth(const OverallHealthStatus& Status)
{
    ...
    std::cout << "   健康: " << Status.HealthyChecks << ", 不健康: " << Status.UnhealthyChecks
              << ", 降级: " << Status.DegradedChecks << ", 严重: " << Status.CriticalChecks
              << std::endl;
}
...
int main()
{
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        std::cout << "手动检查 " << static_cast<int>(Type) << ": "
                  << static_cast<int>(Status.Result) << " - " << Status.Message
                  << " (响应时间: " << Status.ResponseTimeMs << "ms)" << std::endl;
    }
    ...
    for (const auto& Pair : OverallStatus.CheckStatuses)
    {
        ...
        std::cout << "  " << static_cast<int>(Pair.first) << ": " << static_cast<int>(Status.Result)
                  << " - " << Status.Message << " (成功率: " << (Status.SuccessRate * 100) << "%)"
                  << std::endl;
    }
    ...
    for (int i = 0; i < 30; ++i)
    {
        ...
        if (i % 10 == 0)
        {
            ...
            std::cout << "第" << (i + 1)
                      << "秒 - 整体状态: " << static_cast<int>(CurrentStatus.OverallResult) << " ("
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        std::cout << "  " << static_cast<int>(Type) << ": " << "总检查: " << Status.TotalChecks
