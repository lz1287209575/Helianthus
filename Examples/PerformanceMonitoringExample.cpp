...
int main()
{
    ...
    std::cout << "  系统资源:\n";
    std::cout << "    内存使用: " << (SystemMetrics.ResourceStats.MemoryUsageBytes / 1024 / 1024)
              << " MB\n";
    std::cout << "    峰值内存: "
              << (SystemMetrics.ResourceStats.PeakMemoryUsageBytes / 1024 / 1024) << " MB\n";
    ...
    std::cout << "    文件描述符: " << SystemMetrics.ResourceStats.FileDescriptorCount.load()
              << "\n";
    std::cout << "    缓冲区池利用率: "
              << (SystemMetrics.ResourceStats.GetBufferPoolUtilization() * 100) << "%\n";
