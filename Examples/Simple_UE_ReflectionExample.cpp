...
int main()
{
    try
    {
        ...
        { static_cast<UObject*>(Obj)->Name = *static_cast<std::string*>(Value); };
        ...
        { static_cast<UObject*>(Obj)->ID = *static_cast<int32_t*>(Value); };
        ...
        { static_cast<UPlayer*>(Obj)->Health = *static_cast<int32_t*>(Value); };
        ...
        { static_cast<UPlayer*>(Obj)->Speed = *static_cast<float*>(Value); };
        ...
        { return &static_cast<UPlayer*>(Obj)->IsAlive; };
        IsAliveProp.Setter = [](void* Obj, void* Value)
        { static_cast<UPlayer*>(Obj)->IsAlive = *static_cast<bool*>(Value); };
        ...
        for (const auto& Name : UClassNames)
        {
            ...
            if (Info)
            {
                ...
                {
                    const SimpleUPropertyInfo* PropInfo =
                        SimpleUReflectionSystem.GetUPropertyInfo(Name, PropName);
                    if (PropInfo)
                    {
                ...
                {
                    const SimpleUFunctionInfo* FuncInfo =
                        SimpleUReflectionSystem.GetUFunctionInfo(Name, FuncName);
                    if (FuncInfo)
                    {
        ...
        if (PlayerObj)
        {
            ...
            Args.clear();
            void* StatusResult =
                SimpleUReflectionSystem.CallUFunction(PlayerObj, "GetStatus", Args);
            if (StatusResult)
            {
        ...
        // 保存脚本绑定
        bool SaveResult =
            SimpleUReflectionSystem.SaveScriptBindings("simple_ue_bindings.lua", "lua");
        if (SaveResult)
        {
