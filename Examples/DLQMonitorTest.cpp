...
void OnDeadLetterAlert(const DeadLetterAlert& Alert)
{
    H_LOG(MQ,
          ...
          "DLQ告警触发: queue={}, type={}, message={}, currentValue={}, thresholdValue={}, "
          "currentRate={:.2f}, thresholdRate={:.2f}",
          Alert.QueueName,
...
int main()
{
    ...
    // 输出一条结构化日志验证
    Helianthus::Common::StructuredLogger::Log(Helianthus::Common::StructuredLogLevel::INFO,
                                              "MQ",
    ...
    // 配置集群分片/副本，便于演示健康波动与状态导出
    {
        ...
        ShardInfo Shard0;
        Shard0.Id = 0;
        Shard0.Replicas = {{"node-a", ReplicaRole::LEADER, true},
                           {"node-b", ReplicaRole::FOLLOWER, true}};
        ShardInfo Shard1;
        Shard1.Id = 1;
        Shard1.Replicas = {{"node-b", ReplicaRole::LEADER, true},
                           {"node-a", ReplicaRole::FOLLOWER, true}};
    ...
    // 获取一次队列指标进行验证
    {
        ...
        if (Queue->GetQueueMetrics(config.Name, m) == QueueResult::SUCCESS)
        {
            H_LOG(MQ,
                  Helianthus::Common::LogVerbosity::Display,
                  "初始指标: queue={}, pending={}, total={}, processed={}, dlq={}, retried={}, "
    ...
    {
        auto message =
            std::make_shared<Message>(MessageType::TEXT, "正常消息 " + std::to_string(i));
    ...
    {
        auto expiredMessage =
            std::make_shared<Message>(MessageType::TEXT, "过期消息 " + std::to_string(i));
    ...
    {
        H_LOG(
            MQ, Helianthus::Common::LogVerbosity::Display, "清除后活跃告警数量: {}", alerts.size());
    }
    ...
    {
        H_LOG(
            MQ, Helianthus::Common::LogVerbosity::Display, "所有DLQ统计数量: {}", allStats.size());
        for (const auto& stat : allStats)
        {
            H_LOG(MQ,
                  ...
                  "队列: {}, 总死信: {}, 当前死信: {}, 死信率: {:.2f}%",
                  stat.QueueName,
                  stat.TotalDeadLetterMessages,
                  stat.CurrentDeadLetterMessages,
                  stat.DeadLetterRate * 100.0);
        }
    }
    ...
    // 获取最终一次队列指标
    {
        ...
        if (Queue->GetQueueMetrics(config.Name, m) == QueueResult::SUCCESS)
        {
            H_LOG(MQ,
                  Helianthus::Common::LogVerbosity::Display,
                  "最终指标: queue={}, pending={}, total={}, processed={}, dlq={}, retried={}, "
                  "enq_rate={:.2f}/s, deq_rate={:.2f}/s, p50={:.2f}ms, p95={:.2f}ms",
                  m.QueueName,
                  m.PendingMessages,
                  m.TotalMessages,
