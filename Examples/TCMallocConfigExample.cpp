...
void DemoBasicConfig()
{
    ...
    std::cout << "  最大总线程缓存: " << DefaultConfig.MaxTotalThreadCacheBytes / (1024 * 1024) << " MB" << std::endl;
    std::cout << "  最大单线程缓存: " << DefaultConfig.MaxThreadCacheBytes / (1024 * 1024) << " MB" << std::endl;
    std::cout << "  页堆空闲字节: " << DefaultConfig.PageHeapFreeBytes / (1024 * 1024) << " MB" << std::endl;
    ...
    std::cout << "  最大总线程缓存: " << NewConfig.MaxTotalThreadCacheBytes / (1024 * 1024) << " MB" << std::endl;
    std::cout << "  最大单线程缓存: " << NewConfig.MaxThreadCacheBytes / (1024 * 1024) << " MB" << std::endl;
...
void DemoPerformanceConfig()
{
    ...
    // 设置性能优化配置
    bool Result = TCMallocWrapper::SetPerformanceConfig(
        true,         // 启用激进内存释放
        true,         // 启用大内存分配优化
        64 * 1024     // 64KB 大内存阈值
    );
    ...
    // 设置线程缓存配置
    Result = TCMallocWrapper::SetThreadCacheConfig(
        256 * 1024 * 1024,  // 256MB 总缓存
    ...
    // 设置页堆配置
    Result = TCMallocWrapper::SetPageHeapConfig(
        512 * 1024 * 1024,  // 512MB 页堆空闲
...
void DemoDebugConfig()
{
    ...
    // 启用调试模式
    bool Result = TCMallocWrapper::SetDebugConfig(
        true,   // 启用调试模式
        true,   // 启用内存泄漏检查
        10      // 栈深度为 10
    );
    ...
    // 启用采样
    Result = TCMallocWrapper::SetSamplingConfig(
        512 * 1024,  // 512KB 采样率
        true         // 启用采样
    );
...
void DemoRuntimeOperations()
{
    ...
    std::cout << "释放的内存: " << (StatsBefore.HeapSize - StatsAfter.HeapSize) / 1024 << " KB" << std::endl;
