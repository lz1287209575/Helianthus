...
int main()
{
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "测试2：创建、初始化和析构FileBasedPersistence");
    {
        auto filePersistence = std::make_unique<FileBasedPersistence>();
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "FileBasedPersistence创建成功");
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "测试3：在独立线程中创建和析构FileBasedPersistence");
    {
        std::thread destructorThread([]() {
            ...
            if (initResult == QueueResult::SUCCESS) {
                ...
                filePersistence.reset();
                H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "线程内：FileBasedPersistence析构成功");
            }
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "测试4：测试PersistenceManager的析构");
    {
        ...
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "PersistenceManager初始化完成，结果: {}", static_cast<int>(initResult));
