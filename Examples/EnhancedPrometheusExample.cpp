...
static std::string CollectEnhancedMetrics(MessageQueue& MQ, EnhancedPrometheusExporter& EnhancedExporter)
{
...
int main()
{
    MessageQueue MQ;
    MQ.Initialize();
    QueueConfig C; C.Name = "enhanced_metrics_demo"; C.Persistence = PersistenceMode::MEMORY_ONLY;
    MQ.CreateQueue(C);
    ...
    // 启动批处理演示线程
    std::thread([&MQ, &EnhancedExporter]() {
        ...
        for (;;) {
            ...
            if (TxId != 0) {
                ...
                if (MQ.SendMessageInTransaction(TxId, Q, Msg) == QueueResult::SUCCESS) {
                    ...
                    EnhancedExporter.UpdateTransactionPerformance(false, true, false, false, 0, 0);
                }
            }
    ...
    std::thread([&EnhancedExporter]() {
        for (;;) {
            ...
            std::cout << "  Avg Duration: " << BatchStats.GetAverageDurationMs() << " ms" << std::endl;
            std::cout << "  P50 Duration: " << BatchStats.GetP50DurationMs() << " ms" << std::endl;
            std::cout << "  P95 Duration: " << BatchStats.GetP95DurationMs() << " ms" << std::endl;
            std::cout << "  P99 Duration: " << BatchStats.GetP99DurationMs() << " ms" << std::endl;
            
            // 零拷贝统计
            const auto& ZeroCopyStats = EnhancedExporter.GetZeroCopyStats();
            std::cout << "Zero-Copy Stats:" << std::endl;
            std::cout << "  Total Operations: " << ZeroCopyStats.TotalOperations.load() << std::endl;
            std::cout << "  Avg Duration: " << ZeroCopyStats.GetAverageDurationMs() << " ms" << std::endl;
            std::cout << "  P50 Duration: " << ZeroCopyStats.GetP50DurationMs() << " ms" << std::endl;
            std::cout << "  P95 Duration: " << ZeroCopyStats.GetP95DurationMs() << " ms" << std::endl;
            std::cout << "  P99 Duration: " << ZeroCopyStats.GetP99DurationMs() << " ms" << std::endl;
            
            // 事务统计
            const auto& TxStats = EnhancedExporter.GetTransactionStats();
            std::cout << "Transaction Stats:" << std::endl;
            std::cout << "  Total Transactions: " << TxStats.TotalTransactions.load() << std::endl;
            std::cout << "  Committed: " << TxStats.CommittedTransactions.load() << std::endl;
            std::cout << "  Rolled Back: " << TxStats.RolledBackTransactions.load() << std::endl;
            std::cout << "  Success Rate: " << (TxStats.GetSuccessRate() * 100) << "%" << std::endl;
            std::cout << "  Avg Commit Time: " << TxStats.GetAverageCommitTimeMs() << " ms" << std::endl;
            std::cout << "  P95 Commit Time: " << TxStats.GetP95CommitTimeMs() << " ms" << std::endl;
