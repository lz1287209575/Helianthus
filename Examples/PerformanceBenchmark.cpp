...
int main(int argc, char** argv)
{
    ...
    if (argc >= 2)
        MessageCount = static_cast<uint32_t>(std::stoul(argv[1]));
    if (argc >= 3)
        PayloadBytes = static_cast<uint32_t>(std::stoul(argv[2]));
    if (argc >= 4)
        Mode = argv[3];
    if (argc >= 5)
        Runs = static_cast<uint32_t>(std::stoul(argv[4]));
    ...
    auto ToRps = [&](double ms) -> double
    { return (ms > 0.0) ? (MessageCount / (ms / 1000.0)) : 0.0; };
    double AvgRps = ToRps(AvgMs);
    double P50Rps = ToRps(P50Ms);
    double P95Rps = ToRps(P95Ms);

    // 标准化输出：单行 key=value，便于采集
    std::cout << "mode=" << Mode << " message_count=" << MessageCount
              << " payload_bytes=" << PayloadBytes << " runs=" << Runs << " avg_ms=" << AvgMs
              << " p50_ms=" << P50Ms << " p95_ms=" << P95Ms << " avg_rps=" << AvgRps
              << " p50_rps=" << P50Rps << " p95_rps=" << P95Rps << "\n";
