...
public:
    CalculatorService() : RpcServiceBase("CalculatorService")
    {
        ...
        // 注册同步方法
        RegisterMethod(
            "add", [this](const std::string& params) -> std::string { return HandleAdd(params); });
        ...
        RegisterAsyncMethod(
            "asyncAdd",
            [this](const RpcContext& context, const std::string& params, RpcCallback callback)
            { HandleAsyncAdd(context, params, callback); });

        std::cout << "[CalculatorService] 服务初始化完成，注册了 " << GetMethodNames().size()
                  << " 个方法" << std::endl;
    }
...
private:
    ...
    std::string HandleSubtract(const std::string& params)
    {
        ...
        double result = a - b;
        std::cout << "[CalculatorService] subtract(" << a << ", " << b << ") = " << result
                  << std::endl;
        return CreateResult(result);
    }
    ...
    std::string HandleMultiply(const std::string& params)
    {
        ...
        double result = a * b;
        std::cout << "[CalculatorService] multiply(" << a << ", " << b << ") = " << result
                  << std::endl;
        return CreateResult(result);
    }
    ...
    std::string HandleDivide(const std::string& params)
    {
        ...
        double result = a / b;
        std::cout << "[CalculatorService] divide(" << a << ", " << b << ") = " << result
                  << std::endl;
        return CreateResult(result);
    }

    void HandleAsyncAdd(const RpcContext& context, const std::string& params, RpcCallback callback)
    {
        // 模拟异步处理
        std::thread(
...
public:
    StringService() : RpcServiceBase("StringService")
    {
        ...
        RegisterMethod("uppercase",
                       [this](const std::string& params) -> std::string
                       { return HandleUppercase(params); });

        RegisterMethod("lowercase",
                       [this](const std::string& params) -> std::string
                       { return HandleLowercase(params); });

        std::cout << "[StringService] 服务初始化完成，注册了 " << GetMethodNames().size()
                  << " 个方法" << std::endl;
    }
...
private:
    ...
    std::string HandleReverse(const std::string& params)
    {
        ...
        std::cout << "[StringService] reverse(\"" << text << "\") = \"" << reversed << "\""
                  << std::endl;
    ...
    std::string HandleUppercase(const std::string& params)
    {
        ...
        std::cout << "[StringService] uppercase(\"" << text << "\") = \"" << upper << "\""
                  << std::endl;
    ...
    std::string HandleLowercase(const std::string& params)
    {
        ...
        std::cout << "[StringService] lowercase(\"" << text << "\") = \"" << lower << "\""
                  << std::endl;
...
public:
    RpcServerExample()
    {
        ...
        Server->SetErrorHandler(
            [](RpcResult result, const std::string& error) {
                std::cout << "[RpcServer] 错误: " << static_cast<int>(result) << " - " << error
                          << std::endl;
            });

        // 添加中间件
        Server->AddMiddleware(
            [](RpcContext& context) -> bool
            {
                std::cout << "[Middleware] 处理调用: " << context.ServiceName << "."
                          << context.MethodName << std::endl;
                return true;  // 允许继续处理
            });
    }
...
public:
    RpcClientExample()
    {
        ...
        // 设置事件处理器
        Client->SetConnectionStateHandler(
            [](bool connected) {
                std::cout << "[RpcClient] 连接状态: " << (connected ? "已连接" : "已断开")
                          << std::endl;
            });

        Client->SetErrorHandler(
            [](RpcResult result, const std::string& error) {
                std::cout << "[RpcClient] 错误: " << static_cast<int>(result) << " - " << error
                          << std::endl;
            });
    }
    ...
    void TestBatchCalls()
    {
        ...
        for (size_t i = 0; i < testCalls.size(); ++i)
        {
            ...
            std::cout << "[RpcClient] 连续调用 " << (i + 1) << ": " << serviceName << "."
                      << methodName << "(" << params << ") = " << static_cast<int>(callResult)
                      << " - " << result << std::endl;
        }
    }
...
private:
                  ...
                  const std::string& params)
    {
        ...
        std::cout << "[RpcClient] " << serviceName << "." << methodName << "(" << params
    ...
    void TestAsyncCall(const std::string& serviceName,
                       ...
                       const std::string& params)
    {
        RpcResult callResult = Client->CallAsync(
            serviceName,
            methodName,
            params,
            [serviceName, methodName, params](RpcResult result, const std::string& response)
            {
                std::cout << "[RpcClient] 异步调用 " << serviceName << "." << methodName << "("
                          << params << ") = " << static_cast<int>(result) << " - " << response
                          << std::endl;
            },
            5000);
        ...
        {
            std::cout << "[RpcClient] 异步调用启动失败: " << static_cast<int>(callResult)
                      << std::endl;
        }
    }
};
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (arg == "--help" || arg == "-h")
        {
            ...
            std::cout << "  " << argv[0] << " --client --address 127.0.0.1 --port 8080"
                      << std::endl;
            return 0;
        }
    }
