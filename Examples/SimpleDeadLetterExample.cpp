...
int main()
{
    ...
    std::thread initThread([&queue, &initResult]() {
        initResult = queue->Initialize();
    });
    ...
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "发送消息成功 id={}", testMessage->Header.Id);
    }
    ...
    {
        H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "接收到消息 id={}, content={}", 
              receivedMessage->Header.Id, receivedMessage->Payload.AsString());
