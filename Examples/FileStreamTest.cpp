...
int main()
{
    ...
    H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "测试1：基本文件流操作");
    {
        ...
        testFile.open("./test_filestream_data/test.bin",
    ...
    H_LOG(
        MQ, Helianthus::Common::LogVerbosity::Display, "测试2：MockFileBasedPersistence文件流操作");
    {
        ...
        if (mockPersistence->Initialize("./test_filestream_data"))
        {
            H_LOG(MQ,
                  Helianthus::Common::LogVerbosity::Display,
                  "MockFileBasedPersistence初始化成功");

            mockPersistence->TestOperation();
            H_LOG(MQ, Helianthus::Common::LogVerbosity::Display, "测试操作完成");

            H_LOG(
                MQ, Helianthus::Common::LogVerbosity::Display, "开始析构MockFileBasedPersistence");
            mockPersistence.reset();
            H_LOG(
                MQ, Helianthus::Common::LogVerbosity::Display, "MockFileBasedPersistence析构完成");
        }
        else
        {
            H_LOG(
                MQ, Helianthus::Common::LogVerbosity::Error, "MockFileBasedPersistence初始化失败");
        }
    }
        ...
        std::thread fileStreamThread(
            []()
            {
                ...
                if (mockPersistence->Initialize("./test_filestream_thread_data"))
                {
                    H_LOG(MQ,
                          Helianthus::Common::LogVerbosity::Display,
                          "线程内：MockFileBasedPersistence初始化成功");
                ...
                else
                {
                    H_LOG(MQ,
                          ...
                          "线程内：MockFileBasedPersistence初始化失败");
                }
            });
        ...
        std::thread lifecycleThread(
            []()
            {
                ...
                if (mockPersistence->Initialize("./test_filestream_lifecycle_data"))
                {
                    ...
                    H_LOG(
                        MQ, Helianthus::Common::LogVerbosity::Display, "线程内：生命周期测试完成");
                }
                else
                {
                    H_LOG(MQ,
