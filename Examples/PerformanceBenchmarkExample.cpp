...
private:
    ...
    BenchmarkResult RunBasicPerformanceTest()
    {
        ...
        Result.LatencyMs =
            static_cast<double>(Duration.count()) / (MessageCount * 2) / 1e6;  // 发送+接收

        std::cout << "  完成: " << MessageCount << " 消息, " << std::fixed << std::setprecision(2)
                  << Result.Throughput << " msg/s, " << Result.LatencyMs << " ms 平均延迟"
                  << std::endl;
    ...
    BenchmarkResult RunCompressionPerformanceTest()
    {
        ...
        std::cout << "  完成: " << MessageCount << " 消息, " << std::fixed << std::setprecision(2)
                  << Result.Throughput << " msg/s, " << "批处理提升: " << BatchRatio << "x"
                  << std::endl;
    ...
    BenchmarkResult RunEncryptionPerformanceTest()
    {
        ...
        double PriorityOverhead =
            static_cast<double>(PriorityDuration.count()) / NoPriorityDuration.count();

        std::cout << "  完成: " << MessageCount << " 消息, " << std::fixed << std::setprecision(2)
                  << Result.Throughput << " msg/s, " << "优先级开销: " << PriorityOverhead << "x"
                  << std::endl;
    ...
    BenchmarkResult RunBatchProcessingTest()
    {
        ...
        for (uint64_t Batch = 0; Batch < BatchCount; ++Batch)
        {
            ...
            auto BatchDuration =
                std::chrono::duration_cast<std::chrono::nanoseconds>(BatchEndTime - BatchStartTime);

            // 更新增强指标
            EnhancedExporter.UpdateBatchPerformance(
                "benchmark_queue", BatchDuration.count(), BatchSize);
        }
        ...
        std::cout << "  完成: " << TotalMessages << " 消息 (" << BatchCount << " 批次), "
                  << std::fixed << std::setprecision(2) << Result.Throughput << " msg/s, "
                  << Result.LatencyMs << " ms 平均延迟, " << "P95: " << Result.P95LatencyMs << " ms"
                  << std::endl;
    ...
    BenchmarkResult RunZeroCopyTest()
    {
        ...
        for (uint64_t i = 0; i < OperationCount; ++i)
        {
            ...
            auto OpDuration =
                std::chrono::duration_cast<std::chrono::nanoseconds>(OpEndTime - OpStartTime);
        ...
        std::cout << "  完成: " << OperationCount << " 操作, " << std::fixed << std::setprecision(2)
                  << Result.Throughput << " ops/s, " << Result.LatencyMs << " ms 平均延迟, "
    ...
    BenchmarkResult RunTransactionTest()
    {
        ...
        for (uint64_t i = 0; i < TransactionCount; ++i)
        {
            ...
            auto TxDuration =
                std::chrono::duration_cast<std::chrono::nanoseconds>(TxEndTime - TxStartTime);

            // 更新增强指标
            bool Committed = (CommitResult == QueueResult::SUCCESS);
            EnhancedExporter.UpdateTransactionPerformance(
                Committed, !Committed, false, false, CommitDuration.count(), 0);
        }
        ...
        std::cout << "  完成: " << TransactionCount << " 事务, " << std::fixed
                  << std::setprecision(2) << Result.Throughput << " tx/s, " << Result.LatencyMs
                  << " ms 平均延迟, " << "成功率: " << TxStats.GetSuccessRate() * 100 << "%"
                  << std::endl;
    ...
    BenchmarkResult RunConcurrentTest()
    {
        ...
        for (uint64_t ThreadId = 0; ThreadId < ThreadCount; ++ThreadId)
        {
            Threads.emplace_back(
                [this, ThreadId, MessagesPerThread, MessageSize]()
                {
    ...
    void GenerateReport(const std::vector<BenchmarkResult>& Results)
    {
        ...
        std::cout << std::left << std::setw(25) << "测试名称" << std::setw(12) << "消息数"
                  << std::setw(12) << "吞吐量" << std::setw(12) << "平均延迟" << std::setw(12)
                  << "P95延迟" << std::setw(12) << "P99延迟" << std::endl;

        std::cout << std::string(85, '-') << std::endl;

        for (const auto& Result : Results)
        {
            std::cout << std::left << std::setw(25) << Result.TestName << std::setw(12)
                      << Result.MessageCount << std::setw(12) << std::fixed << std::setprecision(0)
                      << Result.Throughput << std::setw(12) << std::fixed << std::setprecision(2)
                      << Result.LatencyMs << std::setw(12) << std::fixed << std::setprecision(2)
                      << Result.P95LatencyMs << std::setw(12) << std::fixed << std::setprecision(2)
                      << Result.P99LatencyMs << std::endl;
        }
        ...
        // 找到最佳性能
        auto MaxThroughput = std::max_element(Results.begin(),
                                              Results.end(),
                                              [](const BenchmarkResult& a, const BenchmarkResult& b)
                                              { return a.Throughput < b.Throughput; });

        auto MinLatency = std::min_element(Results.begin(),
                                           Results.end(),
                                           [](const BenchmarkResult& a, const BenchmarkResult& b)
                                           { return a.LatencyMs < b.LatencyMs; });

        std::cout << "最高吞吐量: " << MaxThroughput->TestName << " (" << std::fixed
                  << std::setprecision(0) << MaxThroughput->Throughput << " msg/s)" << std::endl;
        std::cout << "最低延迟: " << MinLatency->TestName << " (" << std::fixed
                  << std::setprecision(2) << MinLatency->LatencyMs << " ms)" << std::endl;
