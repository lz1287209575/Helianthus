...
public:
    ...
    std::string GetDebugInfo() const
    {
        return Name + " [ID:" + std::to_string(GetObjectId()) + "] " +
               "HP:" + std::to_string(Health) + "/" + std::to_string(MaxHealth) + " " + "POS:(" +
               std::to_string(PositionX) + "," + std::to_string(PositionY) + "," +
               std::to_string(PositionZ) + ")";
    }
};
...
public:
    ...
    void Attack(Entity* Target)
    {
        if (Target && Target->IsActive)
        {
            ...
            std::cout << "⚔️  " << Name << " attacks " << Target->Name << " for " << Damage
                      << " damage!" << std::endl;
...
public:
    ...
    void PerformAttack(Player* Target)
    {
        if (Target && Target->IsActive)
        {
            std::cout << "👹 " << Name << " attacks " << Target->Name << " for " << BaseAttackPower
                      << " damage!" << std::endl;
    ...
    {
        return Name + " [" + EnemyType + "] - HP: " + std::to_string(Health) + "/" +
...
private:
    ...
    static void Demo6_ReflectionIntrospection()
    {
        ...
        std::cout << "\n  属性列表:" << std::endl;
        std::cout
            << "    Level: int [ScriptReadable, BlueprintReadWrite, Category=Progress, SaveGame]"
            << std::endl;
        std::cout << "    Experience: int [ScriptReadable, BlueprintReadWrite, Category=Progress, "
                     "SaveGame]"
                  << std::endl;
        std::cout << "    PlayerClass: std::string [Config, EditAnywhere, Category=Player]"
                  << std::endl;
        std::cout << "    Gold: int [SaveGame, BlueprintReadWrite, Category=Economy]" << std::endl;

        std::cout << "\n  方法列表:" << std::endl;
        std::cout
            << "    AddExperience(int): void [ScriptCallable, BlueprintCallable, Category=Progress]"
            << std::endl;
        std::cout << "    LevelUp(): void [ScriptCallable, BlueprintCallable, Category=Progress]"
                  << std::endl;
        std::cout
            << "    Attack(Entity*): void [ScriptCallable, BlueprintCallable, Category=Combat]"
            << std::endl;
