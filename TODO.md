...
- [x] **压缩/加密统计**：压缩比、加密时间等性能指标 ✅
 - [ ] **属性持久化与入队保留**：确保 `Compressed`、`CompressionAlgorithm`、`Encrypted`、`EncryptionAlgorithm` 在入队/持久化/出队全链路不丢失
 - [ ] **接收侧回环验证通过**：自动解密+解压后与原文一致，解密统计更新
 - [x] **批量基准**：发送N条并输出吞吐与平均耗时 ✅
...
### 高优先级（立即开始）
1. **压缩/加密验证修复**：属性持久化与入队保留、接收侧回环命中
2. **性能测试**：确保系统性能
