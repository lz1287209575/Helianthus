...
- [x] 仅编译变更类：结合 CMake 的对象库或分目标，减少链接时间
- [ ] 生成器健壮性：更复杂签名/模板/默认参数/函数指针解析（进行中）
  - [x] 参数分割：支持模板尖括号/小括号/大括号/字符串字面量
  - [x] 参数名提取：支持函数指针与默认参数
  - [x] 多标签解析修复：RPC 元信息按数组写入而非合并字符串
  - [x] 聚合入口过滤 `Tests/` 路径（仍生成 per-class 片段）
  - [x] 测试目标自动编译链接 `generated/classes/Tests/**` 片段，提供 `RegisterRpc_*` 符号
  - [x] 新增集成测试：多标签筛选、NoAutoRegister、复杂签名（模板/默认参数/函数指针）
  - [ ] 成员函数指针参数解析（如 `void (T:: *Fp)(int)`）
  - [ ] 引用折叠与数组参数名更精准提取
  - [ ] 进一步异常用例覆盖（嵌套模板、属性语法、宏换行）
...
### 进行中
- 生成器健壮性增强：模板/默认参数/函数指针解析与更多边界用例
