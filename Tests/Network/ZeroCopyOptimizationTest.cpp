...
TEST_F(ZeroCopyOptimizationTest, SystemMemoryInfo)
{
    ...
    std::cout << "系统内存信息测试完成，总物理内存: " 
              ...
              << "可用物理内存: " << (MemInfo.AvailablePhysicalMemory / (1024 * 1024)) << " MB" << std::endl;
}
...
TEST_F(ZeroCopyOptimizationTest, PerformanceBenchmark)
{
    ...
    auto TraditionalTime = std::chrono::duration_cast<std::chrono::milliseconds>(EndTime - StartTime);
    ...
    auto MemoryMappedTime = std::chrono::duration_cast<std::chrono::milliseconds>(EndTime - StartTime);
    ...
    {
        std::cout << "内存映射性能提升: " 
                  << (100.0 * (TraditionalTime.count() - MemoryMappedTime.count()) / TraditionalTime.count()) 
                  << "%" << std::endl;
    }
}
