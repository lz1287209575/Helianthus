...
TEST_F(AsyncReadWriteTest, AsyncWriteResume)
{
    ...
    // 模拟 AsyncWrite 续传
    Context.Post(
        [&Context, &Buffer, &WriteCompleted, &TotalWritten, &WriteError]()
        {
            // 模拟分块写入
            size_t ChunkSize = 10;
...
TEST_F(AsyncReadWriteTest, AsyncReadResume)
{
    ...
    // 模拟 AsyncRead 续传
    Context.Post(
        [&Context, &ReadBuffer, &ReadCompleted, &TotalRead, &ReadError, &ExpectedData]()
        {
            // 模拟分块读取
            size_t ChunkSize = 8;
...
TEST_F(AsyncReadWriteTest, ErrorHandlingAndCancel)
{
    ...
    // 模拟错误处理和取消
    Context.Post(
        [&Context, &ErrorCount, &CancelCount]()
        {
            // 模拟网络错误
            ErrorCount++;
...
TEST_F(AsyncReadWriteTest, LargeDataTransfer)
{
    ...
    // 模拟大块数据传输
    Context.Post(
        [&Context, &LargeData, &TransferCompleted, &TotalTransferred]()
        {
            // 模拟分块传输
            ...
            while (Transferred < TotalSize && !TransferCompleted.load())
            {
                ...
                if (Transferred + CurrentChunk >= TotalSize)
                {
                    ...
                    std::cout << "大块数据传输完成，总共传输 " << Transferred << " 字节"
                              << std::endl;
                }
                else
                {
                    ...
                    {
                        std::cout << "大块数据传输进度: " << (Transferred / (1024 * 1024)) << "MB"
                                  << std::endl;
                    }
