...
TEST_F(ReactorTest, AddTest)
{
    ...
    bool addResult = Reactor->Add(testFd, EventMask::Read, [&callbackCalled](EventMask mask) {
        std::cout << "Reactor callback called with mask: " << static_cast<int>(mask) << std::endl;
        callbackCalled = true;
    });
