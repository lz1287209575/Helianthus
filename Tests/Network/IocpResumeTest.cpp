    ...
    if (ConnectResult == Helianthus::Network::NetworkError::NONE)
    {
        ...
        ContextPtr->Post(
            [ContextPtr,
             ...
             ServerAddr]()
            {
                AsyncSocket->AsyncSend(LargeData.data(),
                                       LargeData.size(),
                                       ServerAddr,
                                       [&WriteCompleted, &TotalWritten, &WriteError](
                                           Helianthus::Network::NetworkError Error, size_t Bytes)
                                       {
                                           WriteError = Error;
                                           TotalWritten.store(Bytes);
                                           WriteCompleted.store(true);
                                           std::cout << "IOCP AsyncWrite 完成，总共写入 " << Bytes
                                                     << " 字节，错误: " << static_cast<int>(Error)
                                                     << std::endl;
                                       });
            });
    ...
    if (ConnectResult == Helianthus::Network::NetworkError::NONE)
    {
        ...
        ContextPtr->Post(
            [ContextPtr,
             ...
             ServerAddr]()
            {
                AsyncSocket->AsyncReceive(ReadBuffer.data(),
                                          ReadBuffer.size(),
                                          [&ReadCompleted, &TotalRead, &ReadError](
...
TEST_F(IocpResumeTest, IocpCancelOperation)
{
    ...
    // 模拟长时间操作然后取消
    ContextPtr->Post(
        [ContextPtr, &OperationCancelled, &CancelError, AsyncSocket]()
        {
            // 模拟一个长时间的操作
    ...
    EXPECT_EQ(CancelError, Helianthus::Network::NetworkError::CONNECTION_CLOSED)
        << "应该返回连接关闭错误";
    EXPECT_TRUE(StopCalled.load()) << "事件循环应该停止";
