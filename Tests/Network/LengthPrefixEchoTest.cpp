...
void ReadExact(std::shared_ptr<ReadState> State,
...
protected:
    ...
    void TearDown() override {
        if (ServerContext) ServerContext->Stop();
        if (ClientContext) ClientContext->Stop();
...
TEST_F(LengthPrefixEchoTest, SingleMessage)
{
    ...
    Acceptor->AsyncAccept([
        ...
        ReadExact(HeaderState, [this, ServerSocket, HeaderState, &Done](NetworkError Err2){
            ...
            ReadExact(BodyState, [ServerSocket, BodyState, &Done](NetworkError Err3){
    ...
    ReadExact(EchoHeader, [Client, &EchoDone, &Echoed, EchoHeader](NetworkError ErrH){
        ASSERT_EQ(ErrH, NetworkError::NONE);
        uint32_t NetLen2 = 0; std::memcpy(&NetLen2, EchoHeader->Buffer->data(), 4);
        ...
        ReadExact(EchoBody, [&EchoDone, &Echoed, EchoBody](NetworkError ErrB){
            ASSERT_EQ(ErrB, NetworkError::NONE);
            Echoed.assign(EchoBody->Buffer->data(), EchoBody->Buffer->data() + EchoBody->TargetSize);
            EchoDone = true;
        });
    });
...
TEST_F(LengthPrefixEchoTest, FragmentedMessage)
{
    ...
    Acceptor->AsyncAccept([
        ...
        ReadExact(HeaderState, [this, ServerSocket, HeaderState, &Done](NetworkError Err2){
            ASSERT_EQ(Err2, NetworkError::NONE);
            uint32_t NetLen = 0; std::memcpy(&NetLen, HeaderState->Buffer->data(), 4);
            uint32_t BodyLen = ntohl(NetLen);
            auto BodyState = std::make_shared<ReadState>();
            BodyState->Socket = ServerSocket;
            BodyState->Buffer = std::make_shared<std::vector<char>>(BodyLen);
