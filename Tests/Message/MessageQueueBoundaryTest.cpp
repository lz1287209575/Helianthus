...
TEST_F(MessageQueueBoundaryTest, ZeroBatchSize)
{
    ...
    EXPECT_EQ(MQ.ReceiveBatchMessages(TestQueueName, ReceivedMessages, 0, 100), QueueResult::INVALID_PARAMETER);
...
TEST_F(MessageQueueBoundaryTest, ExtremePriorityValues)
{
    ...
    std::cout << "发送低优先级消息，优先级值: " << static_cast<int>(Message1->Header.Priority) << std::endl;
    std::cout << "发送高优先级消息，优先级值: " << static_cast<int>(Message2->Header.Priority) << std::endl;
    ...
    if (ReceivedMessage) {
        std::cout << "第一次接收消息优先级: " << static_cast<int>(ReceivedMessage->Header.Priority) << std::endl;
    }
    ...
    if (ReceivedMessage) {
        std::cout << "第二次接收消息优先级: " << static_cast<int>(ReceivedMessage->Header.Priority) << std::endl;
    }
...
TEST_F(MessageQueueBoundaryTest, SingleThreadStress)
{
    ...
    EXPECT_TRUE(FinalReceiveResult == QueueResult::SUCCESS || FinalReceiveResult == QueueResult::TIMEOUT);
}
...
TEST_F(MessageQueueBoundaryTest, InvalidTransactionId)
{
    ...
    EXPECT_EQ(MQ.SendMessageInTransaction(InvalidId, TestQueueName, Message), QueueResult::TRANSACTION_NOT_FOUND);
