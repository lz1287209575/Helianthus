...
TEST_F(TransactionTest, AcknowledgeMessageInTransaction)
{
    ...
    Result =
        MQ.AcknowledgeMessageInTransaction(TxId, "transaction_test_queue", ReceivedMsg->Header.Id);
...
TEST_F(TransactionTest, RejectMessageInTransaction)
{
    ...
    Result = MQ.RejectMessageInTransaction(
        TxId, "transaction_test_queue", ReceivedMsg->Header.Id, "test reject");
...
TEST_F(TransactionTest, ConcurrentTransactions)
{
    ...
    for (int ThreadId = 0; ThreadId < ThreadCount; ++ThreadId)
    {
        Threads.emplace_back(
            [this, ThreadId, TransactionsPerThread, &SuccessCount]()
            {
                for (int i = 0; i < TransactionsPerThread; ++i)
                {
                    ...
                    QueueResult Result =
                        MQ.SendMessageInTransaction(TxId, "transaction_test_queue", Msg);
