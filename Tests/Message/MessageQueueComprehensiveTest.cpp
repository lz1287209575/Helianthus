...
TEST_F(MessageQueueComprehensiveTest, QueueCreationAndManagement)
{
    ...
    // 测试重复创建
    EXPECT_EQ(LocalMQ.CreateQueue(Config), QueueResult::QUEUE_NOT_FOUND);
...
TEST_F(MessageQueueComprehensiveTest, BatchMessageOperations)
{
    ...
    EXPECT_EQ(MQ.ReceiveBatchMessages(TestQueueName, ReceivedMessages, 10, 1000),
              QueueResult::SUCCESS);
    ...
    for (size_t i = 0; i < 5; ++i)
        expected.insert("batch_msg_" + std::to_string(i));
    for (const auto& m : ReceivedMessages)
    {
        actual.insert(std::string(m->Payload.Data.begin(), m->Payload.Data.end()));
    }
...
TEST_F(MessageQueueComprehensiveTest, BatchReceiveWithTimeout)
{
    ...
    EXPECT_EQ(MQ.ReceiveBatchMessages(TestQueueName, ReceivedMessages, 10, 100),
              QueueResult::SUCCESS);
...
TEST_F(MessageQueueComprehensiveTest, QueueCapacityLimits)
{
    ...
    // 尝试发送更多消息应该失败
    EXPECT_EQ(MQ.SendMessage("small_queue", CreateTestMessage("overflow")),
...
TEST_F(MessageQueueComprehensiveTest, EmptyQueueOperations)
{
    ...
    // 测试空队列的Peek
    EXPECT_EQ(MQ.PeekMessage(TestQueueName, ReceivedMessage), QueueResult::TIMEOUT);
    EXPECT_EQ(ReceivedMessage, nullptr);
}
...
TEST_F(MessageQueueComprehensiveTest, InvalidOperations)
{
    ...
    // 测试发送到不存在的队列
    EXPECT_EQ(MQ.SendMessage("non_existent_queue", CreateTestMessage("test")),
              QueueResult::QUEUE_NOT_FOUND);
    ...
    for (int i = 0; i < ThreadCount; ++i)
    {
        SendThreads.emplace_back(
            [&, i]()
            {
                ...
                {
                    auto Message = CreateTestMessage("thread_" + std::to_string(i) + "_msg_" +
                                                     std::to_string(j));
...
TEST_F(MessageQueueComprehensiveTest, ErrorHandling)
{
    ...
    EXPECT_EQ(UninitializedMQ.SendMessage(TestQueueName, Message), QueueResult::QUEUE_NOT_FOUND);
    EXPECT_EQ(UninitializedMQ.ReceiveMessage(TestQueueName, Message), QueueResult::QUEUE_NOT_FOUND);
...
TEST_F(MessageQueueComprehensiveTest, StatisticsTracking)
{
    ...
    {
        EXPECT_EQ(
            MQ.SendMessage(TestQueueName, CreateTestMessage("stats_msg_" + std::to_string(i))),
            QueueResult::SUCCESS);
    }
    ...
    EXPECT_GE(Stats.TotalMessages, 5);
    EXPECT_GE(Stats.PendingMessages, 0);
    EXPECT_GE(Stats.TotalBytes, 0);
    EXPECT_GE(Stats.ProcessedMessages, 0);
}
...
TEST_F(MessageQueueComprehensiveTest, ConsumerRegistration)
{
    ...
    {
        EXPECT_EQ(
            MQ.SendMessage(TestQueueName, CreateTestMessage("consumer_msg_" + std::to_string(i))),
            QueueResult::SUCCESS);
    }
...
TEST_F(MessageQueueComprehensiveTest, ProducerRegistration)
{
    ...
    // 仅校验接口返回码
    MQ.GetActiveProducers(TestQueueName);
...
TEST_F(MessageQueueComprehensiveTest, MonitoringAndMetrics)
{
    ...
    {
        EXPECT_EQ(
            MQ.SendMessage(TestQueueName, CreateTestMessage("metric_msg_" + std::to_string(i))),
            QueueResult::SUCCESS);
    }
    ...
    EXPECT_GE(PerfStats.BatchOperations, 0);
    EXPECT_GE(PerfStats.ZeroCopyOperations, 0);

    // 获取事务统计
    TransactionStats TxStats;
    EXPECT_EQ(MQ.GetTransactionStats(TxStats), QueueResult::SUCCESS);
    EXPECT_GE(TxStats.TotalTransactions, 0);
...
TEST_F(MessageQueueComprehensiveTest, QueueCleanup)
{
    ...
    {
        EXPECT_EQ(
            MQ.SendMessage(TestQueueName, CreateTestMessage("cleanup_msg_" + std::to_string(i))),
            QueueResult::SUCCESS);
    }
