...
{
    std::unordered_map<std::string, std::string> TestObject = {{"key1", "value1"},
                                                               {"key2", "value2"}};
...
TEST_F(ConfigManagerTest, ChangeCallback)
{
    ...
    ConfigChangeCallback Callback =
        [&](const std::string& Key, const ConfigValue& Old, const ConfigValue& New)
    {
        ChangedKey = Key;
        OldValue = Old;
        NewValue = New;
...
TEST_F(ConfigManagerTest, LoadMonitoringConfig)
{
    ...
    EXPECT_EQ(Manager->GetString("monitoring.tracing_endpoint"),
