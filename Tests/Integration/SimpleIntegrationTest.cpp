...
protected:
    void SetUp() override {
        Queue = std::make_unique<Helianthus::MessageQueue::MessageQueue>("simple_integration_test", 100);
    }
...
TEST_F(SimpleIntegrationTest, BatchOperations) {
    ...
    for (int i = 0; i < BatchSize; ++i) {
        auto Message = Helianthus::MessageQueue::Message::Create("test", "Batch Message " + std::to_string(i));
...
TEST_F(SimpleIntegrationTest, PerformanceStats) {
    ...
    for (int i = 0; i < MessageCount; ++i) {
        auto Message = Helianthus::MessageQueue::Message::Create("test", "Stats Message " + std::to_string(i));
...
TEST_F(SimpleIntegrationTest, ConcurrentAccess) {
    ...
    for (int t = 0; t < ThreadCount; ++t) {
        Threads.emplace_back([this, t, MessagesPerThread, &SuccessCount]() {
            for (int i = 0; i < MessagesPerThread; ++i) {
                auto Message = Helianthus::MessageQueue::Message::Create("test", 
