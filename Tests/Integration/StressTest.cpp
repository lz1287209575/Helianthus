...
TEST_F(StressTest, HighConcurrencyReceiveTest) {
    ...
    for (int i = 0; i < TotalMessages; ++i) {
        auto Message = std::make_shared<Helianthus::MessageQueue::Message>(
            Helianthus::MessageQueue::MessageType::TEXT,
            "Preload Message " + std::to_string(i));
...
TEST_F(StressTest, TransactionStressTest) {
    ...
    auto TransactionWorker = [&](int ThreadId) {
        for (int i = 0; i < TransactionCount; ++i) {
            auto TransactionId = Queue->BeginTransaction("stress_tx_" + std::to_string(ThreadId), 1000);
            ...
            for (int j = 0; j < MessagesPerTransaction; ++j) {
                auto Message = std::make_shared<Helianthus::MessageQueue::Message>(
                    Helianthus::MessageQueue::MessageType::TEXT,
                    "TX " + std::to_string(ThreadId) + " " + std::to_string(i) + " " + std::to_string(j));
    ...
    // 验证结果
    EXPECT_EQ(CommitSuccessCount.load() + CommitFailureCount.load() + RollbackCount.load(), TotalTransactions);
    ...
    auto MemoryWorker = [&](int ThreadId) {
        for (int iter = 0; iter < IterationCount; ++iter) {
            ...
            for (int i = 0; i < MessagesPerIteration; ++i) {
                auto Message = std::make_shared<Helianthus::MessageQueue::Message>(
                    Helianthus::MessageQueue::MessageType::TEXT,
                    "Memory " + std::to_string(ThreadId) + " " + std::to_string(iter) + " " + std::to_string(i));
